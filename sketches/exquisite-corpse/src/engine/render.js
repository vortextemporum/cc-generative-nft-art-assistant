...
function deriveEdgeProfile(hash) {
  ...
  // Consume more random values for noise seed derivation
  const noiseSeed = hash.slice(2, 18);
  const techniques = {
...
function sampleEdge(profile, yNorm) {
  ...
  const yIdx = yNorm * (anchors.length - 1);
  const lo = Math.floor(yIdx);
...
function buildField(leftProfile, rightProfile, ownHash) {
  ...
  const leftNoise = createNoise2D(initRandom(leftProfile.noiseSeed + 'a'.repeat(32)));
  const rightNoise = createNoise2D(initRandom(rightProfile.noiseSeed + 'b'.repeat(32)));
