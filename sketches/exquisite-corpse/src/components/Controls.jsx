...
export default function Controls({ chain, setChain, view, setView }) {
  const addPiece = () => {
  ...
  const regenerateAll = () => {
    setChain(chain.map(() => generateHash()));
  };
  ...
  const saveGallery = () => {
    ...
    link.download = `exquisite-corpse-${chain.length}pcs-${Date.now()}.png`;
