...
function drawNotationMarks() {
  ...
  // Dynamic markings - varying sizes
  const dynamics = [
    "ppp", "pp", "p", "mp", "mf", "f", "ff", "fff",
    "sfz", "sfp", "sfpp", "fp", "fz", "rf", "rfz",
    "cresc.", "decresc.", "dim.", "morendo", "smorzando",
    "più f", "più p", "poco f", "poco p", "subito p", "subito f"
  ];
  textStyle(ITALIC);

  const numDynamics = Math.floor(rndInt(5, 12) * features.densityValue);
  for (let i = 0; i < numDynamics; i++) {
    const x = rnd(MARGIN + 50, WIDTH - MARGIN - 60);
    const y = rnd(MARGIN + 20, HEIGHT - MARGIN - 20);
    // Vary size from 8 to 18 pixels
    const size = rnd(8, 18) * scaleFactor;
    textSize(size);
    fill(features.palette.inkLight + (size > 14 ? "" : "cc"));
...
function drawMetronomeMarks() {
  // Metronome marking near tempo
  const noteValues = ["𝅗𝅥", "♩", "♪", "𝅘𝅥𝅮"];
  const bpm = rndInt(40, 180);
  const note = rndChoice(noteValues);

  textSize(11 * scaleFactor);
  fill(features.palette.ink);
  noStroke();
  text(`${note} = ${bpm}`, MARGIN + 130 * scaleFactor, MARGIN - 28 * scaleFactor);

  // Occasionally add duration/timing marks
  if (rndBool(0.5)) {
    const durations = ["ca. 3'", "ca. 5'", "ca. 7-10'", "∞", "~2'30\""];
    textSize(9 * scaleFactor);
    fill(features.palette.inkLight);
    text(rndChoice(durations), WIDTH - MARGIN - 60 * scaleFactor, HEIGHT - MARGIN + 15 * scaleFactor);
  }
}
