...
function drawPolymorphicTextureFields(voice, section) {
  // Fields of texture representing timbral areas
  const numFields = Math.max(1, Math.floor(rnd(1, 4) * features.densityValue));
...
function drawPolymorphicTransformChains(voice, section) {
  // Chains showing transformation from one state to another
  const numChains = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));

  for (let c = 0; c < numChains; c++) {
    const startX = rnd(section.xStart + 20, section.xEnd - 120);
    const y = rnd(voice.yStart + 20, voice.yEnd - 20);
...
function drawPolymorphicSoundMasses(voice, section) {
  // Dense sound masses - Logothetis' klangmassen
  const numMasses = Math.max(1, Math.floor(rnd(1, 4) * features.densityValue));

  for (let m = 0; m < numMasses; m++) {
    const x = rnd(section.xStart + 40, section.xEnd - 40);
    const y = rnd(voice.yStart + 25, voice.yEnd - 25);
    const w = rnd(30, 60) * scaleFactor;
...
function drawPolymorphicExpandingForms(voice, section) {
  // Forms that expand outward
  const numForms = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));
...
function drawPolymorphicRotationalGlyphs(voice, section) {
  // Glyphs with rotational symmetry
  const numGlyphs = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));
...
function drawTheaterActionSymbols(voice, section) {
  // Symbols representing theatrical actions
  const numActions = Math.max(2, Math.floor(rnd(4, 9) * features.densityValue));
...
function drawTheaterObjectNotation(voice, section) {
  // Notation for objects/props used in performance
  const objects = ["chair", "table", "book", "glass", "bell", "paper", "hat", "cane"];
  const numObjects = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));
...
function drawTheaterSpatialPositions(voice, section) {
  // Stage positions marked with circles/labels
...
function drawTheaterLightingCues(voice, section) {
  // Lighting cue markers
  const cues = ["LX1", "LX2", "LX3", "FADE", "SPOT", "DIM", "BLACK", "UP"];
  const numCues = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));
...
function drawTheaterAbsurdistMarks(voice, section) {
  // Absurdist/Dada-inspired marks
  const numMarks = Math.max(2, Math.floor(rnd(4, 8) * features.densityValue));
...
function drawCuesConditionalBrackets(voice, section) {
  // Brackets indicating conditional sections (if/then/else)
  const numBrackets = Math.max(1, Math.floor(rnd(1, 4) * features.densityValue));
