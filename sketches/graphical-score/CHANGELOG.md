...
**Palindrome Mirror Point Now Uses Variable Position**

Fixed `applyPalindromeSymmetry()` which was hard-coded to mirror at `WIDTH/2` regardless of the `mirrorPoint` feature value (0.3-0.7). Now correctly uses `features.mirrorPoint` to determine the mirror axis position.

- Mirror axis can now appear anywhere from 30% to 70% across the score
- Visual mirror line is drawn at the correct position
- Content is properly flipped around the variable mirror point
...
**Spiral/Crumb Mode Placement Variety**

Added `getSpiralPlacements()` system that probabilistically varies spiral positioning instead of always centering. Spirals can now appear in various arrangements:

| Pattern | Probability | Description |
|---------|-------------|-------------|
| Centered | 30% | Original behavior - single centered spiral |
| Offset | 10% | Shifted to one side (left/right/top/bottom) |
| Diagonal corner | 10% | Positioned in a corner region |
| Paired horizontal | 10% | Two spirals side by side |
| Paired vertical | 10% | Two spirals stacked vertically |
| Triple diagonal | 10% | Three spirals in diagonal arrangement |
| Scattered | 10% | 3-4 small spirals at random positions |
| Peripheral | 10% | Near edge rather than center |

Updated functions to use placement system:
- `drawSpiralPaths()` - Basic Archimedean spiral
- `drawCircularNotation()` - Concentric circles with radial divisions
- `drawSpiralVariants()` - Logarithmic, double, galaxy, Fermat spirals
- `drawMandalaPattern()` - Multi-layer rotational symmetry patterns
- `drawSpiralText()` - Text along spiral path

Each placement includes a scale factor so multiple spirals are appropriately sized.
