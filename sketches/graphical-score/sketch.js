...
function drawPolymorphicLogothetisGlyphs(voice, section) {
  ...
  const numGlyphs = Math.max(2, Math.floor(rnd(4, 10) * features.densityValue));
...
function drawTheaterStageDirections(voice, section) {
  ...
  const numDirections = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));
...
function drawCuesCueSymbols(voice, section) {
  ...
  const numCues = Math.max(2, Math.floor(rnd(3, 8) * features.densityValue));
...
function drawIndigenousGatheringCircles(voice, section) {
  // Circles representing gathering places
  const numCircles = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));

  for (let c = 0; c < numCircles; c++) {
    const x = rnd(section.xStart + 35, section.xEnd - 35);
    const y = rnd(voice.yStart + 25, voice.yEnd - 25);
    const size = rnd(30, 50) * scaleFactor;
...
function drawDeepListeningMandalaCircles(voice, section) {
  // Mandala-like concentric circles
  const x = (section.xStart + section.xEnd) / 2;
  const y = voice.yCenter;
  const maxSize = Math.min(60, (voice.yEnd - voice.yStart) / 2 - 5) * scaleFactor;
  const numRings = rndInt(4, 8);
...
function drawSoundMapRiverLines(voice, section) {
  // River/stream flowing lines
  const numRivers = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));
  ...
  for (let r = 0; r < numRivers; r++) {
    ...
    for (let s = 0; s < rndInt(6, 10); s++) {
      rx += rnd(15, 30) * scaleFactor;
      ry += rnd(-15, 15) * scaleFactor;
      // Keep in bounds
