/**
 * Graphical Score v3.30.0
 * A generative graphical score with 21 distinct modes inspired by
 * 20th/21st century avant-garde composers
 *
...
const PALETTES = {
  ...
  polymorphicOrange: {
    // Logothetis' distinctive orange/sepia printing - Vienna 1960s
    paper: "#faf5e8", paperDark: "#f0e8d0",
    ink: "#c45a20", inkLight: "#d87840",
    accent: "#8b3a00", faded: "#e8a060",
    type: "authentic"
  },

  theaterRed: {
    // Kagel's theatrical aesthetic - dramatic red/black for stage directions
    paper: "#f8f4f0", paperDark: "#e8e0d8",
    ink: "#1a1a1a", inkLight: "#4a4a4a",
    accent: "#8b0000", faded: "#b89080",
    type: "authentic"
  },

  cuesGreen: {
    // Christian Wolff's clean, participatory aesthetic - green for go/cue
    paper: "#f5f8f5", paperDark: "#e0e8e0",
    ink: "#1a2a1a", inkLight: "#4a5a4a",
    accent: "#2a6a2a", faded: "#8ab08a",
    type: "authentic"
  },

  indigenousTurquoise: {
    // Raven Chacon's Indigenous aesthetic - turquoise, earth, sky
    paper: "#faf8f0", paperDark: "#e8e0d0",
    ink: "#2a2018", inkLight: "#5a4a38",
    accent: "#2a8a8a", faded: "#a0b8b0",
    type: "authentic"
  }
};
...
const MODES = {
  ...
  polymorphic: {
    name: "Polymorphic",
    composer: "Logothetis",
    description: "Unique symbol system with 80+ invented signs, elastic time, gravity-influenced shapes",
    weight: 0.05,
    elements: [
      "logothetisGlyphs", "elasticTime", "gravityShapes", "integrationSigns",
      "dynamicCurves", "textureFields", "transformChains", "densityGradients",
      "soundMasses", "articulationSymbols", "connectionWebs", "layeredPlanes",
      "rhythmicCells", "expandingForms", "compressingForms", "rotationalGlyphs"
    ],
    prefersPalette: ["polymorphicOrange", "sepia", "manuscript"]
  },

  theater: {
    name: "Theater",
    composer: "Kagel",
    description: "Instrumental theater with absurdist actions, stage directions, object notation",
    weight: 0.05,
    elements: [
      "stageDirections", "actionSymbols", "objectNotation", "movementArrows",
      "speechBubbles", "timedActions", "spatialPositions", "gestureLines",
      "propsSymbols", "entranceExits", "lightingCues", "absurdistMarks",
      "synchronizationLines", "performerPaths", "audienceMarks", "durationBars"
    ],
    prefersPalette: ["theaterRed", "manuscript", "blueprint"]
  },

  cues: {
    name: "Cues",
    composer: "Christian Wolff",
    description: "Participatory cueing system, performer interdependence, conditional instructions",
    weight: 0.05,
    elements: [
      "cueSymbols", "conditionalBrackets", "responseMarkers", "listeningIndicators",
      "waitSigns", "coordinationLines", "playerNumbers", "reactionArrows",
      "holdSymbols", "releaseMarks", "attentionPoints", "flexibleDurations",
      "muteSymbols", "playSymbols", "pauseIndicators", "groupBrackets"
    ],
    prefersPalette: ["cuesGreen", "manuscript", "blueprint"]
  },

  indigenous: {
    name: "Indigenous",
    composer: "Raven Chacon",
    description: "Flag-scores, land-based symbols, Indigenous perspectives on sound and space",
    weight: 0.05,
    elements: [
      "flagSymbols", "landMarkers", "windDirections", "horizonLines",
      "celestialMarks", "waterSymbols", "fireSymbols", "earthSymbols",
      "pathwayLines", "gatheringCircles", "cardinalPoints", "seasonalMarks",
      "silenceSpaces", "voiceContours", "drumPatterns", "breathMarks"
    ],
    prefersPalette: ["indigenousTurquoise", "sepia", "manuscript"]
  },

  deepListening: {
    name: "DeepListening",
    composer: "Pauline Oliveros",
    description: "Meditation-based scores, mandala patterns, breath and awareness notation",
    weight: 0.05,
    elements: [
      "mandalaCircles", "breathCycles", "awarenessFoci", "concentricRings",
      "meditationText", "expansionArrows", "contractionArrows", "centerPoints",
      "radiatingLines", "pulseMarks", "attentionZones", "silenceFields",
      "listeningDepths", "sonicEnvironment", "bodyAwareness", "groupConnections"
    ],
    prefersPalette: ["cageTransparent", "manuscript", "parchment"]
  },
  ...
  soundMap: {
    ...
    weight: 0.05,
    elements: [
      "riverLines", "topographicContours", "locationMarkers", "soundSources",
      "flowArrows", "confluencePoints", "watershedBoundaries", "elevationMarks",
      "fieldRecordingSymbols", "environmentalZones", "migrationPaths", "echoLocations",
      "densityGradients", "temporalLayers", "spatialCoordinates", "listeningPosts"
    ],
    prefersPalette: ["blueprint", "upicBlue", "manuscript"]
  }
};
...
function drawTextInstructions(voice, section) {
  ...
  const numTexts = Math.max(3, Math.floor(rnd(4, 8) * features.densityValue));
  ...
  for (let i = 0; i < numTexts; i++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 20 + ((i + 0.5) / numTexts) * (section.width - 100) + rnd(-10, 10) * scaleFactor;
    const y = rnd(voice.yStart + 15, voice.yEnd - 15);
...
function drawStripsodyOnomatopoeia(voice, section) {
  ...
  const numSounds = Math.max(3, Math.floor(rnd(5, 9) * features.densityValue));

  noStroke();

  for (let i = 0; i < numSounds; i++) {
    // Distribute evenly across section (horizontal) and voice (vertical) with jitter
    const x = section.xStart + 30 + ((i + 0.5) / numSounds) * (section.width - 90) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter for even spread
    const yBase = voice.yStart + 20 + ((i % 3) / 2) * (voice.height - 40);
    const y = yBase + rnd(-10, 10) * scaleFactor;
...
function drawStripsodyBubbles(voice, section) {
  // Speech/thought bubbles - distribute across section
  const numBubbles = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));

  for (let i = 0; i < numBubbles; i++) {
    // Distribute evenly across section (horizontal) and voice (vertical) with jitter
    const cx = section.xStart + 40 + ((i + 0.5) / numBubbles) * (section.width - 80) + rnd(-15, 15) * scaleFactor;
    const cyBase = voice.yStart + 25 + ((i % 3) / 2) * (voice.height - 50);
    const cy = cyBase + rnd(-10, 10) * scaleFactor;
    const w = rnd(30, 70) * scaleFactor;
    const h = rnd(20, 40) * scaleFactor;
...
function drawStripsodyExplosions(voice, section) {
  // Explosion burst shapes (comic book style) - distribute across section
  const colors = features.palette.colors || ["#dd2222", "#ff8800", "#ffcc00"];
  const numExplosions = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));

  for (let i = 0; i < numExplosions; i++) {
    // Distribute evenly across section (horizontal) and voice (vertical) with jitter
    const cx = section.xStart + 40 + ((i + 0.5) / numExplosions) * (section.width - 80) + rnd(-15, 15) * scaleFactor;
    const cyBase = voice.yStart + 25 + ((i % 3) / 2) * (voice.height - 50);
    const cy = cyBase + rnd(-10, 10) * scaleFactor;
...
function drawStripsodyStars(voice, section) {
  // Impact stars (4-8 pointed stars) - distribute across section
  const colors = features.palette.colors || ["#ffcc00", "#ffffff", "#ff6600"];
  const numStars = Math.max(4, Math.floor(rnd(5, 10) * features.densityValue));

  for (let i = 0; i < numStars; i++) {
    // Distribute evenly across section (horizontal) and voice (vertical) with jitter
    const cx = section.xStart + 20 + ((i + 0.5) / numStars) * (section.width - 40) + rnd(-10, 10) * scaleFactor;
    const cyBase = voice.yStart + 15 + ((i % 3) / 2) * (voice.height - 30);
    const cy = cyBase + rnd(-8, 8) * scaleFactor;
    const size = rnd(8, 20) * scaleFactor;
...
function drawStripsodyFaces(voice, section) {
  // Simple expressive faces (emoticons) - distribute across section
  const numFaces = Math.max(3, Math.floor(rnd(4, 7) * features.densityValue));
  ...
  for (let i = 0; i < numFaces; i++) {
    // Distribute evenly across section (horizontal) and voice (vertical) with jitter
    const cx = section.xStart + 30 + ((i + 0.5) / numFaces) * (section.width - 60) + rnd(-10, 10) * scaleFactor;
    const cyBase = voice.yStart + 20 + ((i % 3) / 2) * (voice.height - 40);
    const cy = cyBase + rnd(-8, 8) * scaleFactor;
...
function drawStripsodyLightning(voice, section) {
  // Lightning bolt shapes - distribute across section
  const colors = features.palette.colors || ["#ffcc00", "#ff6600", "#ffffff"];
  const numBolts = Math.max(3, Math.floor(rnd(4, 7) * features.densityValue));

  for (let i = 0; i < numBolts; i++) {
    // Distribute evenly across section (horizontal) and voice (vertical) with jitter
    const x = section.xStart + 25 + ((i + 0.5) / numBolts) * (section.width - 50) + rnd(-10, 10) * scaleFactor;
    const yBase = voice.yStart + 10 + ((i % 3) / 2) * (voice.height - 35);
    const y = yBase + rnd(-8, 8) * scaleFactor;
    const size = rnd(15, 30) * scaleFactor;
...
function drawStripsodySpirals(voice, section) {
  // Dizzy spiral marks - distribute across section
  ...
  for (let i = 0; i < numSpirals; i++) {
    // Distribute evenly across section with jitter
    const cx = section.xStart + 20 + ((i + 0.5) / numSpirals) * (section.width - 40) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const cyBase = voice.yStart + 15 + ((i % 3) / 2) * (voice.height - 30);
    const cy = cyBase + rnd(-8, 8) * scaleFactor;
    const maxR = rnd(8, 18) * scaleFactor;
    const turns = rnd(2, 4);
...
function drawStripsodyDroplets(voice, section) {
  // Sweat/water droplets - distribute across section
  const colors = features.palette.colors || ["#66ccff", "#3399ff", "#99ddff"];
  const numDrops = Math.max(3, Math.floor(rnd(3, 7) * features.densityValue));

  noStroke();

  for (let i = 0; i < numDrops; i++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 15 + ((i + 0.5) / numDrops) * (section.width - 30) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 15 + ((i % 3) / 2) * (voice.height - 30);
    const y = yBase + rnd(-8, 8) * scaleFactor;
    const size = rnd(5, 12) * scaleFactor;
...
function drawStripsodyPuffs(voice, section) {
  // Smoke/cloud puffs - distribute across section
  const numPuffs = Math.max(2, Math.floor(rnd(2, 4) * features.densityValue));
  ...
  for (let p = 0; p < numPuffs; p++) {
    // Distribute evenly across section with jitter
    const cx = section.xStart + 30 + ((p + 0.5) / numPuffs) * (section.width - 60) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter
    const cyBase = voice.yStart + 20 + ((p % 3) / 2) * (voice.height - 40);
    const cy = cyBase + rnd(-10, 10) * scaleFactor;
    const size = rnd(15, 30) * scaleFactor;
...
function drawStripsodyImpact(voice, section) {
  // Impact radiating lines (from central point) - distribute across section
  const numImpacts = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));

  stroke(features.palette.ink);

  for (let i = 0; i < numImpacts; i++) {
    // Distribute evenly across section with jitter
    const cx = section.xStart + 30 + ((i + 0.5) / numImpacts) * (section.width - 60) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter for even spread
    const cyBase = voice.yStart + 20 + ((i % 3) / 2) * (voice.height - 40);
    const cy = cyBase + rnd(-10, 10) * scaleFactor;
...
function drawStripsodyWobble(voice, section) {
  // Wobble/vibration marks (wavy lines around objects) - distribute across section
  ...
  for (let w = 0; w < numWobbles; w++) {
    // Distribute evenly across section with jitter
    const cx = section.xStart + 25 + ((w + 0.5) / numWobbles) * (section.width - 50) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const cyBase = voice.yStart + 15 + ((w % 3) / 2) * (voice.height - 30);
    const cy = cyBase + rnd(-8, 8) * scaleFactor;
...
function drawAnkhrasmationDurations(voice, section) {
  // Duration symbols - colored bars of varying lengths - distribute across section
  const colors = features.palette.colors ||
    ["#cc3300", "#0066cc", "#ffcc00", "#00aa55", "#9933cc"];

  const numUnits = Math.max(4, Math.floor(rnd(6, 12) * features.densityValue));

  noStroke();

  for (let i = 0; i < numUnits; i++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 15 + ((i + 0.5) / numUnits) * (section.width - 55) + rnd(-10, 10) * scaleFactor;
    const y = rnd(voice.yStart + 10, voice.yEnd - 10);
...
function drawBraxtonDiagrams(voice, section) {
  // Diagrammatic/schematic notation - distribute across section
  const numDiagrams = Math.max(3, Math.floor(rnd(4, 7) * features.densityValue));
  ...
  for (let i = 0; i < numDiagrams; i++) {
    // Distribute evenly across section with jitter
    const cx = section.xStart + 40 + ((i + 0.5) / numDiagrams) * (section.width - 80) + rnd(-10, 10) * scaleFactor;
    const cy = rnd(voice.yStart + 25, voice.yEnd - 25);
...
function drawMobileFloatingCells(voice, section) {
  ...
  const numCells = Math.max(3, Math.floor(rnd(5, 9) * features.densityValue));

  for (let i = 0; i < numCells; i++) {
    // Distribute evenly across section with some jitter
    const cx = section.xStart + 30 + ((i + 0.5) / numCells) * (section.width - 60) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter for even spread
    const cyBase = voice.yStart + 20 + ((i % 3) / 2) * (voice.height - 40);
    const cy = cyBase + rnd(-10, 10) * scaleFactor;
    const w = rnd(30, 80) * scaleFactor;
...
function drawMobileBranches(voice, section) {
  // Branching mobile arm structures - distribute across section
  const numBranches = Math.max(2, Math.floor(rnd(2, 4) * features.densityValue));
  ...
  for (let i = 0; i < numBranches; i++) {
    // Distribute evenly across section with jitter
    const startX = section.xStart + 30 + ((i + 0.5) / numBranches) * (section.width - 60) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter
    const startYBase = voice.yStart + 20 + ((i % 3) / 2) * (voice.height - 40);
    const startY = startYBase + rnd(-10, 10) * scaleFactor;
...
function drawMobileCellClusters(voice, section) {
  // Groups of cells clustered together - distribute across section
  const numClusters = Math.max(2, Math.floor(rnd(1, 3) * features.densityValue));

  for (let c = 0; c < numClusters; c++) {
    // Distribute evenly across section with jitter
    const clusterX = section.xStart + 50 + ((c + 0.5) / numClusters) * (section.width - 100) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter
    const clusterYBase = voice.yStart + 30 + ((c % 3) / 2) * (voice.height - 60);
    const clusterY = clusterYBase + rnd(-10, 10) * scaleFactor;
...
function drawMobilePivotPoints(voice, section) {
  // Central pivot points with radiating elements - distribute across section
  const numPivots = Math.max(2, Math.floor(rnd(2, 4) * features.densityValue));

  for (let i = 0; i < numPivots; i++) {
    // Distribute evenly across section with jitter
    const px = section.xStart + 40 + ((i + 0.5) / numPivots) * (section.width - 80) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter
    const pyBase = voice.yStart + 25 + ((i % 3) / 2) * (voice.height - 50);
    const py = pyBase + rnd(-10, 10) * scaleFactor;
...
function drawMobileBalancedArms(voice, section) {
  // Horizontal balanced arm structures - distribute across section
  const numArms = Math.max(2, Math.floor(rnd(2, 4) * features.densityValue));
  ...
  for (let i = 0; i < numArms; i++) {
    // Distribute evenly across section with jitter
    const cx = section.xStart + 60 + ((i + 0.5) / numArms) * (section.width - 120) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter
    const cyBase = voice.yStart + 20 + ((i % 3) / 2) * (voice.height - 40);
    const cy = cyBase + rnd(-10, 10) * scaleFactor;
...
function drawMobileRotatingGroups(voice, section) {
  // Groups of elements that could rotate around a central axis - distribute across section
  const numGroups = Math.max(2, Math.floor(rnd(1, 3) * features.densityValue));

  for (let g = 0; g < numGroups; g++) {
    // Distribute evenly across section with jitter
    const cx = section.xStart + 50 + ((g + 0.5) / numGroups) * (section.width - 100) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter
    const cyBase = voice.yStart + 35 + ((g % 3) / 2) * (voice.height - 70);
    const cy = cyBase + rnd(-10, 10) * scaleFactor;
...
// ============================================================
// POLYMORPHIC MODE (Logothetis) - v3.24.0
// Anestis Logothetis developed a unique symbol system with 80+ invented
// signs representing sound parameters. His "polymorphic" scores allow
// multiple interpretations while maintaining structural relationships.
// ============================================================

function drawPolymorphicLogothetisGlyphs(voice, section) {
  // Logothetis invented 80+ unique symbols - these are inspired by his vocabulary
  const numGlyphs = Math.max(4, Math.floor(rnd(6, 12) * features.densityValue));

  for (let g = 0; g < numGlyphs; g++) {
    // Distribute evenly across section with some jitter
    const x = section.xStart + 20 + ((g + 0.5) / numGlyphs) * (section.width - 40) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 15 + ((g % 3) / 2) * (voice.height - 30);
    const y = yBase + rnd(-8, 8) * scaleFactor;
    const size = rnd(15, 35) * scaleFactor;
    const glyphType = rndInt(0, 11);
    ...
    switch (glyphType) {
      case 0: // Ascending wedge with stem
        line(0, size / 2, 0, -size / 2);
        triangle(-size / 3, 0, size / 3, 0, 0, -size / 2);
        break;
      case 1: // Descending wedge with stem
        line(0, -size / 2, 0, size / 2);
        triangle(-size / 3, 0, size / 3, 0, 0, size / 2);
        break;
      case 2: // Wave with terminal
        beginShape();
        for (let i = 0; i <= 4; i++) {
          const wx = -size / 2 + (i * size / 4);
          const wy = sin(i * PI) * size / 4;
          vertex(wx, wy);
        }
        endShape();
        ellipse(size / 2, 0, 4 * scaleFactor, 4 * scaleFactor);
        break;
      case 3: // Integration sign (area under curve)
        beginShape();
        vertex(-size / 2, size / 3);
        bezierVertex(-size / 4, -size / 3, size / 4, -size / 3, size / 2, size / 3);
        endShape();
        // Fill under
        fill(features.palette.ink + "20");
        beginShape();
        ...
        noFill();
        break;
      case 4: // Spiral with arrow
        noFill();
        beginShape();
        for (let a = 0; a < TWO_PI * 1.5; a += 0.2) {
          const r = size / 6 + a * size / 15;
          vertex(cos(a) * r, sin(a) * r);
        }
        endShape();
        // Arrow at end
        const endA = TWO_PI * 1.5;
        const endR = size / 6 + endA * size / 15;
        const ex = cos(endA) * endR;
        const ey = sin(endA) * endR;
        fill(features.palette.ink);
        triangle(ex, ey, ex - 4 * scaleFactor, ey - 2 * scaleFactor, ex - 4 * scaleFactor, ey + 2 * scaleFactor);
        noFill();
        break;
      case 5: // Bracket with dots
        line(-size / 2, -size / 3, -size / 2, size / 3);
        line(-size / 2, -size / 3, -size / 3, -size / 3);
        line(-size / 2, size / 3, -size / 3, size / 3);
        fill(features.palette.ink);
        for (let d = 0; d < 3; d++) {
          ellipse(size / 4, -size / 4 + d * size / 4, 3 * scaleFactor, 3 * scaleFactor);
        }
        noFill();
        break;
      case 6: // Expanding form
        for (let e = 0; e < 4; e++) {
          const ew = size / 4 + e * size / 6;
          const eh = size / 8 + e * size / 10;
          arc(-size / 3 + e * size / 5, 0, ew, eh, -HALF_PI, HALF_PI);
        }
        break;
      case 7: // Cross with circles
        line(-size / 2, 0, size / 2, 0);
        line(0, -size / 2, 0, size / 2);
        ellipse(-size / 2, 0, 6 * scaleFactor, 6 * scaleFactor);
        ellipse(size / 2, 0, 6 * scaleFactor, 6 * scaleFactor);
        ellipse(0, -size / 2, 6 * scaleFactor, 6 * scaleFactor);
        ellipse(0, size / 2, 6 * scaleFactor, 6 * scaleFactor);
        break;
      case 8: // Cluster density mark
        fill(features.palette.ink + "30");
        ellipse(0, 0, size, size * 0.6);
        noFill();
        // Internal texture lines
        for (let l = 0; l < 5; l++) {
          const lx = rnd(-size / 3, size / 3);
          const ly = rnd(-size / 5, size / 5);
          line(lx - 3 * scaleFactor, ly, lx + 3 * scaleFactor, ly);
        }
        break;
      case 9: // Stepped contour
        beginShape();
        ...
        endShape();
        break;
      case 10: // Double curve
        beginShape();
        vertex(-size / 2, 0);
        bezierVertex(-size / 4, -size / 2, size / 4, -size / 2, size / 2, 0);
        endShape();
        beginShape();
        vertex(-size / 2, 0);
        bezierVertex(-size / 4, size / 2, size / 4, size / 2, size / 2, 0);
        endShape();
        break;
      case 11: // Arrow complex
        line(-size / 2, 0, size / 3, 0);
        line(size / 3, 0, size / 6, -size / 4);
        line(size / 3, 0, size / 6, size / 4);
        // Secondary arrow
        stroke(features.palette.ink + "80");
        line(-size / 4, size / 3, size / 4, -size / 3);
        break;
    }
...
function drawPolymorphicElasticTime(voice, section) {
  // Time stretches and compresses - represented by variable-width bands
  const numBands = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let b = 0; b < numBands; b++) {
    const startX = section.xStart + rnd(20, 50);
    const endX = section.xEnd - rnd(20, 50);
    const y = rnd(voice.yStart + 20, voice.yEnd - 20);

    // Variable height representing elastic time
    stroke(features.palette.ink + "60");
    strokeWeight(0.8 * scaleFactor);
    noFill();
    ...
    for (let x = startX; x <= endX; x += 8) {
      const stretch = sin((x - startX) * 0.03) * 15 + rnd(-3, 3);
      vertex(x, y + stretch * scaleFactor);
    }
    endShape();
    ...
    beginShape();
    for (let x = startX; x <= endX; x += 8) {
      const stretch = sin((x - startX) * 0.03 + PI) * 15 + rnd(-3, 3);
      vertex(x, y + 20 * scaleFactor + stretch * scaleFactor);
    }
    endShape();

    // Vertical divisions at stress points
    stroke(features.palette.ink + "40");
    for (let d = 0; d < rndInt(3, 7); d++) {
      const dx = rnd(startX + 30, endX - 30);
      const topStretch = sin((dx - startX) * 0.03) * 15;
      const botStretch = sin((dx - startX) * 0.03 + PI) * 15;
      line(dx, y + topStretch * scaleFactor, dx, y + 20 * scaleFactor + botStretch * scaleFactor);
    }
  }
}

function drawPolymorphicGravityShapes(voice, section) {
  // Shapes influenced by gravity - heavier at bottom
  const numShapes = Math.max(1, Math.floor(rnd(3, 7) * features.densityValue));

  for (let s = 0; s < numShapes; s++) {
    const x = rnd(section.xStart + 30, section.xEnd - 30);
    const y = rnd(voice.yStart + 25, voice.yEnd - 25);
    const baseWidth = rnd(20, 40) * scaleFactor;
    const height = rnd(30, 60) * scaleFactor;

    stroke(features.palette.ink);
    strokeWeight(1.2 * scaleFactor);

    // Heavier/wider at bottom
    const topWidth = baseWidth * rnd(0.3, 0.6);
    const bottomWidth = baseWidth * rnd(1.2, 1.8);

    // Draw trapezoid-like shape
    noFill();
    beginShape();
    ...
    // Weight indicator lines at bottom
    stroke(features.palette.ink + "60");
    strokeWeight(0.5 * scaleFactor);
    for (let w = 0; w < 3; w++) {
      const wy = y + height / 2 - w * 4 * scaleFactor;
      const ww = bottomWidth - w * 6 * scaleFactor;
      line(x - ww / 2, wy, x + ww / 2, wy);
    }
  }
}

function drawPolymorphicIntegrationSigns(voice, section) {
  // Mathematical integral-like signs representing cumulative sound
  const numSigns = Math.max(1, Math.floor(rnd(2, 6) * features.densityValue));

  for (let s = 0; s < numSigns; s++) {
    const x = rnd(section.xStart + 25, section.xEnd - 25);
    const y = rnd(voice.yStart + 20, voice.yEnd - 20);
    const height = rnd(25, 50) * scaleFactor;
    ...
    // Integral sign shape
    beginShape();
    vertex(x + 5 * scaleFactor, y - height / 2);
    bezierVertex(
      x - 8 * scaleFactor, y - height / 3,
      x - 8 * scaleFactor, y + height / 3,
      x + 5 * scaleFactor, y + height / 2
    );
    endShape();
    ...
    stroke(features.palette.ink + "80");
    strokeWeight(0.8 * scaleFactor);
    line(x - 10 * scaleFactor, y - height / 2, x + 15 * scaleFactor, y - height / 2);
    line(x - 10 * scaleFactor, y + height / 2, x + 15 * scaleFactor, y + height / 2);

    // Content area
    if (rndBool(0.5)) {
      fill(features.palette.ink + "15");
      noStroke();
      rect(x + 15 * scaleFactor, y - height / 2, rnd(20, 40) * scaleFactor, height, 2 * scaleFactor);
    }
  }
}

function drawPolymorphicDynamicCurves(voice, section) {
  // Curves representing dynamic changes
  const numCurves = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let c = 0; c < numCurves; c++) {
    const startX = rnd(section.xStart + 15, section.xEnd - 80);
    const endX = startX + rnd(60, 120);
    const y = rnd(voice.yStart + 15, voice.yEnd - 15);

    // Dynamic curve with varying thickness
    noFill();
    for (let t = 0; t < 1; t += 0.05) {
      const x1 = lerp(startX, endX, t);
      const x2 = lerp(startX, endX, t + 0.05);
      const curve1 = sin(t * PI * 2) * 20 * scaleFactor;
      const curve2 = sin((t + 0.05) * PI * 2) * 20 * scaleFactor;

      // Thickness varies with dynamic
      const thick = 0.5 + abs(sin(t * PI)) * 2;
      stroke(features.palette.ink);
      strokeWeight(thick * scaleFactor);
      line(x1, y + curve1, x2, y + curve2);
    }

    // Dynamic indicator marks
    stroke(features.palette.accent);
    strokeWeight(0.8 * scaleFactor);
    fill(features.palette.accent + "40");

    // pp mark at thin point
    textSize(8 * scaleFactor);
    noStroke();
    fill(features.palette.ink);
    text("p", startX, y - 25 * scaleFactor);

    // ff mark at thick point
    text("f", (startX + endX) / 2, y - 25 * scaleFactor);
  }
}

function drawPolymorphicTextureFields(voice, section) {
  // Fields of texture representing timbral areas - distribute across section
  const numFields = Math.max(2, Math.floor(rnd(1, 4) * features.densityValue));

  for (let f = 0; f < numFields; f++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 30 + ((f + 0.5) / numFields) * (section.width - 90) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 25 + ((f % 3) / 2) * (voice.height - 50);
    const y = yBase + rnd(-10, 10) * scaleFactor;
    const w = rnd(40, 80) * scaleFactor;
    const h = rnd(30, 50) * scaleFactor;
    ...
    beginShape();
    for (let a = 0; a < TWO_PI; a += 0.3) {
      const r = (a < PI ? w : w * 0.8) / 2 + rnd(-5, 5) * scaleFactor;
      const rh = (a > HALF_PI && a < PI + HALF_PI ? h : h * 0.9) / 2 + rnd(-3, 3) * scaleFactor;
      vertex(x + cos(a) * r, y + sin(a) * rh);
    }
    endShape(CLOSE);

    // Internal texture
    const textureType = rndInt(0, 3);
    stroke(features.palette.ink + "40");
    strokeWeight(0.5 * scaleFactor);
    ...
    if (textureType === 0) {
      // Dots
      for (let d = 0; d < 15; d++) {
        const dx = x + rnd(-w / 3, w / 3);
        const dy = y + rnd(-h / 3, h / 3);
        point(dx, dy);
      }
    } else if (textureType === 1) {
      // Short lines
      for (let l = 0; l < 8; l++) {
        const lx = x + rnd(-w / 3, w / 3);
        const ly = y + rnd(-h / 3, h / 3);
        const la = rnd(0, TWO_PI);
        line(lx, ly, lx + cos(la) * 5 * scaleFactor, ly + sin(la) * 5 * scaleFactor);
      }
    } else if (textureType === 2) {
      // Crosshatch
      for (let l = 0; l < 5; l++) {
        const ly = y - h / 3 + l * h / 7;
        line(x - w / 3, ly, x + w / 3, ly);
      }
      for (let l = 0; l < 5; l++) {
        const lx = x - w / 3 + l * w / 7;
        line(lx, y - h / 3, lx, y + h / 3);
      }
    } else {
      // Wavy
      for (let l = 0; l < 4; l++) {
        const ly = y - h / 4 + l * h / 5;
        beginShape();
        for (let wx = x - w / 3; wx <= x + w / 3; wx += 5) {
          vertex(wx, ly + sin(wx * 0.2) * 3 * scaleFactor);
        }
        endShape();
      }
    }
...
function drawPolymorphicTransformChains(voice, section) {
  // Chains showing transformation from one state to another - distribute across section
  const numChains = Math.max(2, Math.floor(rnd(1, 3) * features.densityValue));

  for (let c = 0; c < numChains; c++) {
    // Distribute evenly across section with jitter
    const startX = section.xStart + 20 + ((c + 0.5) / numChains) * (section.width - 140) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 20 + ((c % 3) / 2) * (voice.height - 40);
    const y = yBase + rnd(-10, 10) * scaleFactor;
    const numSteps = rndInt(3, 6);
    const stepWidth = rnd(25, 40) * scaleFactor;

    for (let s = 0; s < numSteps; s++) {
      const sx = startX + s * stepWidth;
      ...
      const cornerRadius = (1 - progress) * 10 * scaleFactor;
      const size = 15 * scaleFactor;

      if (progress < 0.5) {
        // More circular
        ellipse(sx, y, size * (2 - progress), size * (2 - progress));
      } else {
        // More square
        rect(sx - size / 2, y - size / 2, size, size, cornerRadius);
      }
      ...
      if (s < numSteps - 1) {
        stroke(features.palette.ink + "60");
        strokeWeight(0.8 * scaleFactor);
        line(sx + size / 2 + 3 * scaleFactor, y, sx + stepWidth - size / 2 - 3 * scaleFactor, y);
        // Arrowhead
        fill(features.palette.ink + "60");
        triangle(
          sx + stepWidth - size / 2 - 3 * scaleFactor, y,
          sx + stepWidth - size / 2 - 8 * scaleFactor, y - 3 * scaleFactor,
          sx + stepWidth - size / 2 - 8 * scaleFactor, y + 3 * scaleFactor
        );
        noFill();
      }
...
function drawPolymorphicDensityGradients(voice, section) {
  // Gradients showing density changes
  const numGradients = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));

  for (let g = 0; g < numGradients; g++) {
    const startX = rnd(section.xStart + 20, section.xEnd - 80);
    const width = rnd(60, 100) * scaleFactor;
    const y = rnd(voice.yStart + 20, voice.yEnd - 20);
    const height = rnd(20, 35) * scaleFactor;

    // Draw density gradient with increasing dot density
    for (let x = startX; x < startX + width; x += 3 * scaleFactor) {
      const density = (x - startX) / width;
      ...
      for (let d = 0; d < numDots; d++) {
        const dy = y + rnd(-height / 2, height / 2);
        point(x + rnd(-1, 1) * scaleFactor, dy);
      }
    }
    ...
    stroke(features.palette.ink + "40");
    strokeWeight(0.5 * scaleFactor);
    line(startX, y - height / 2, startX + width, y - height / 2);
    line(startX, y + height / 2, startX + width, y + height / 2);
  }
}

function drawPolymorphicSoundMasses(voice, section) {
  // Dense sound masses - Logothetis' klangmassen - distribute across section
  const numMasses = Math.max(2, Math.floor(rnd(1, 4) * features.densityValue));

  for (let m = 0; m < numMasses; m++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 40 + ((m + 0.5) / numMasses) * (section.width - 80) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 25 + ((m % 3) / 2) * (voice.height - 50);
    const y = yBase + rnd(-10, 10) * scaleFactor;
    const w = rnd(30, 60) * scaleFactor;
    const h = rnd(25, 45) * scaleFactor;

    // Dense filled mass
    fill(features.palette.ink + "25");
    stroke(features.palette.ink);
    strokeWeight(1.2 * scaleFactor);
    ...
    for (let a = 0; a < TWO_PI; a += 0.2) {
      const r = (w / 2 + h / 2) / 2;
      const rVar = r + sin(a * 3) * r * 0.2 + rnd(-3, 3) * scaleFactor;
      vertex(x + cos(a) * rVar * (w / h), y + sin(a) * rVar);
    }
    endShape(CLOSE);

    // Internal density marks
    noFill();
    stroke(features.palette.ink + "50");
    strokeWeight(0.5 * scaleFactor);
    for (let i = 0; i < 8; i++) {
      const ix = x + rnd(-w / 3, w / 3);
      const iy = y + rnd(-h / 3, h / 3);
      const il = rnd(3, 8) * scaleFactor;
      line(ix - il / 2, iy, ix + il / 2, iy);
    }
  }
}

function drawPolymorphicArticulationSymbols(voice, section) {
  // Various articulation symbols from Logothetis' vocabulary
  const numSymbols = Math.max(2, Math.floor(rnd(4, 10) * features.densityValue));

  for (let s = 0; s < numSymbols; s++) {
    const x = rnd(section.xStart + 15, section.xEnd - 15);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    const size = rnd(8, 16) * scaleFactor;
    const symType = rndInt(0, 7);
    ...
    switch (symType) {
      ...
      case 2: // Tenuto dash
        ...
        break;
      case 3: // Marcato
        line(x, y - size / 2, x - size / 2, y + size / 3);
        line(x, y - size / 2, x + size / 2, y + size / 3);
        break;
      case 4: // Sforzando
        fill(features.palette.ink);
      ...
      case 5: // Tremolo
        for (let t = 0; t < 3; t++) {
          line(x - size / 2 + t * 2 * scaleFactor, y - size / 4, x + size / 2 + t * 2 * scaleFactor, y + size / 4);
        }
        break;
      case 6: // Fermata
        arc(x, y, size, size * 0.7, PI, TWO_PI);
        fill(features.palette.ink);
        ellipse(x, y - 2 * scaleFactor, 3 * scaleFactor, 3 * scaleFactor);
        break;
      case 7: // Breath mark
        ...
        bezierVertex(x - size / 4, y - size / 3, x + size / 4, y - size / 3, x + size / 4, y + size / 3);
        endShape();
        break;
    }
  }
...
function drawPolymorphicConnectionWebs(voice, section) {
  // Web-like connections between elements
  const numNodes = Math.max(3, Math.floor(rnd(5, 10) * features.densityValue));
  const nodes = [];
  ...
  for (let n = 0; n < numNodes; n++) {
    nodes.push({
      x: rnd(section.xStart + 20, section.xEnd - 20),
      y: rnd(voice.yStart + 15, voice.yEnd - 15)
    });
  }
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      ...
      if (dist < 80 * scaleFactor && rndBool(0.6)) {
  ...
  for (const node of nodes) {
    if (rndBool(0.5)) {
      fill(features.palette.ink);
      ellipse(node.x, node.y, 4 * scaleFactor, 4 * scaleFactor);
    } else {
      noFill();
      ellipse(node.x, node.y, 6 * scaleFactor, 6 * scaleFactor);
    }
  }
}

function drawPolymorphicLayeredPlanes(voice, section) {
  // Overlapping transparent planes representing layers
  const numPlanes = Math.max(2, Math.floor(rnd(2, 5) * features.densityValue));

  for (let p = 0; p < numPlanes; p++) {
    const x = rnd(section.xStart + 20, section.xEnd - 60);
    const y = rnd(voice.yStart + 15, voice.yEnd - 30);
    const w = rnd(40, 70) * scaleFactor;
    const h = rnd(25, 40) * scaleFactor;
    const rotation = rnd(-0.2, 0.2);
    ...
    textSize(8 * scaleFactor);
    textAlign(CENTER, CENTER);
    text(String.fromCharCode(65 + p), 0, 0); // A, B, C, etc.

    pop();
  }
}

function drawPolymorphicRhythmicCells(voice, section) {
  // Boxed rhythmic cells with internal patterns
  const numCells = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let c = 0; c < numCells; c++) {
    const x = rnd(section.xStart + 15, section.xEnd - 50);
    const y = rnd(voice.yStart + 15, voice.yEnd - 25);
    const w = rnd(35, 55) * scaleFactor;
    const h = rnd(20, 35) * scaleFactor;
    ...
    // Internal rhythm pattern
    const numBeats = rndInt(3, 7);
    const beatWidth = w / (numBeats + 1);

    for (let b = 1; b <= numBeats; b++) {
      const bx = x + b * beatWidth;
      const beatType = rndInt(0, 3);

      if (beatType === 0) {
        // Vertical line
        line(bx, y + 4 * scaleFactor, bx, y + h - 4 * scaleFactor);
      } else if (beatType === 1) {
        // Dot
        fill(features.palette.ink);
        ellipse(bx, y + h / 2, 4 * scaleFactor, 4 * scaleFactor);
        noFill();
      } else if (beatType === 2) {
        // Short dash
        line(bx - 3 * scaleFactor, y + h / 2, bx + 3 * scaleFactor, y + h / 2);
      } else {
        // Rest (empty space with small mark)
        stroke(features.palette.ink + "60");
        line(bx, y + h / 2 - 2 * scaleFactor, bx, y + h / 2 + 2 * scaleFactor);
        stroke(features.palette.ink);
      }
    }
  }
...
function drawPolymorphicExpandingForms(voice, section) {
  // Forms that expand outward - distribute across section
  const numForms = Math.max(2, Math.floor(rnd(2, 5) * features.densityValue));

  for (let f = 0; f < numForms; f++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 35 + ((f + 0.5) / numForms) * (section.width - 70) + rnd(-15, 15) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 25 + ((f % 3) / 2) * (voice.height - 50);
    const y = yBase + rnd(-10, 10) * scaleFactor;
    const maxSize = rnd(25, 45) * scaleFactor;
    const numRings = rndInt(3, 6);

    stroke(features.palette.ink);
    noFill();

    for (let r = 0; r < numRings; r++) {
      const size = (r + 1) / numRings * maxSize;
      strokeWeight((numRings - r) * 0.3 * scaleFactor);
      ...
      if (rndBool(0.7)) {
        ...
        // Expanding squares
        rect(x - size, y - size * 0.75, size * 2, size * 1.5);
      }
    }
    ...
    stroke(features.palette.ink + "60");
    strokeWeight(0.8 * scaleFactor);
    line(x + maxSize + 5 * scaleFactor, y, x + maxSize + 15 * scaleFactor, y);
    fill(features.palette.ink + "60");
    triangle(
      x + maxSize + 15 * scaleFactor, y,
      x + maxSize + 10 * scaleFactor, y - 3 * scaleFactor,
      x + maxSize + 10 * scaleFactor, y + 3 * scaleFactor
    );
  }
}

function drawPolymorphicCompressingForms(voice, section) {
  // Forms that compress inward
  const numForms = Math.max(1, Math.floor(rnd(2, 4) * features.densityValue));

  for (let f = 0; f < numForms; f++) {
    const x = rnd(section.xStart + 35, section.xEnd - 35);
    const y = rnd(voice.yStart + 25, voice.yEnd - 25);
    const maxSize = rnd(25, 45) * scaleFactor;
    const numRings = rndInt(3, 6);
    ...
    for (let r = numRings - 1; r >= 0; r--) {
      const size = (r + 1) / numRings * maxSize;
      strokeWeight((r + 1) * 0.3 * scaleFactor);
      ellipse(x, y, size * 2, size * 1.5);
    }
    ...
    stroke(features.palette.ink + "60");
    strokeWeight(0.8 * scaleFactor);
    line(x - maxSize - 15 * scaleFactor, y, x - maxSize - 5 * scaleFactor, y);
    fill(features.palette.ink + "60");
    triangle(
      x - maxSize - 5 * scaleFactor, y,
      x - maxSize - 10 * scaleFactor, y - 3 * scaleFactor,
      x - maxSize - 10 * scaleFactor, y + 3 * scaleFactor
    );
...
function drawPolymorphicRotationalGlyphs(voice, section) {
  // Glyphs with rotational symmetry - distribute across section
  const numGlyphs = Math.max(2, Math.floor(rnd(2, 5) * features.densityValue));

  for (let g = 0; g < numGlyphs; g++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 30 + ((g + 0.5) / numGlyphs) * (section.width - 60) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 25 + ((g % 3) / 2) * (voice.height - 50);
    const y = yBase + rnd(-10, 10) * scaleFactor;
    const size = rnd(18, 35) * scaleFactor;
    const numArms = rndInt(3, 8);
    const armType = rndInt(0, 3);
    ...
    for (let a = 0; a < numArms; a++) {
      ...
      if (armType === 0) {
        ...
        // Lines with terminal dots
        line(x, y, endX, endY);
        fill(features.palette.ink);
        ellipse(endX, endY, 4 * scaleFactor, 4 * scaleFactor);
        noFill();
      } else if (armType === 2) {
        // Curved arms
        const ctrlX = x + cos(angle + 0.3) * size * 0.7;
        const ctrlY = y + sin(angle + 0.3) * size * 0.7;
        beginShape();
        vertex(x, y);
        ...
        // Arrow arms
        line(x, y, endX, endY);
        const arrowAngle = angle + PI * 0.85;
        line(endX, endY, endX + cos(arrowAngle) * 5 * scaleFactor, endY + sin(arrowAngle) * 5 * scaleFactor);
        const arrowAngle2 = angle - PI * 0.85;
        line(endX, endY, endX + cos(arrowAngle2) * 5 * scaleFactor, endY + sin(arrowAngle2) * 5 * scaleFactor);
      }
    }
...
// ============================================================
// THEATER MODE (Kagel) - v3.25.0
// Mauricio Kagel pioneered instrumental theater, combining music
// with absurdist theatrical actions. His scores include stage
// directions, object notation, and performer movement instructions.
// ============================================================

function drawTheaterStageDirections(voice, section) {
  // Stage directions in italicized text boxes
  const directions = [
    "enter slowly", "pause, look up", "turn to audience",
    "whisper", "freeze", "gesture widely", "exit backwards",
    "approach stand", "bow slightly", "stumble", "recover",
    "stare blankly", "point upward", "shake head", "nod",
    "walk in circle", "stop abruptly"
  ];

  const numDirections = Math.max(2, Math.floor(rnd(4, 7) * features.densityValue));

  for (let d = 0; d < numDirections; d++) {
    // Distribute evenly across section with some jitter
    const x = section.xStart + 20 + ((d + 0.5) / numDirections) * (section.width - 80) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 15 + ((d % 3) / 2) * (voice.height - 30);
    const y = yBase + rnd(-8, 8) * scaleFactor;
    const direction = directions[rndInt(0, directions.length - 1)];
    ...
    textSize(9 * scaleFactor);
    textStyle(ITALIC);
    const tw = textWidth(direction) + 10 * scaleFactor;
    const th = 14 * scaleFactor;

    rect(x, y - th / 2, tw, th, 2 * scaleFactor);

    // Direction text
    noStroke();
    fill(features.palette.accent);
    textAlign(LEFT, CENTER);
    text(direction, x + 5 * scaleFactor, y);
    textStyle(NORMAL);
  }
}

function drawTheaterActionSymbols(voice, section) {
  // Symbols representing theatrical actions - distribute across section
  const numActions = Math.max(3, Math.floor(rnd(4, 9) * features.densityValue));

  for (let a = 0; a < numActions; a++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 20 + ((a + 0.5) / numActions) * (section.width - 40) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 15 + ((a % 3) / 2) * (voice.height - 30);
    const y = yBase + rnd(-8, 8) * scaleFactor;
    const size = rnd(12, 24) * scaleFactor;
    const actionType = rndInt(0, 7);
    ...
    switch (actionType) {
      case 0: // Hand symbol
        // Palm
        ellipse(x, y + size / 4, size * 0.6, size * 0.4);
        // Fingers
        for (let f = 0; f < 5; f++) {
          const fx = x - size / 4 + f * size / 8;
          line(fx, y + size / 8, fx + rnd(-2, 2) * scaleFactor, y - size / 3);
        }
        break;
      case 1: // Eye symbol
        ...
        break;
      case 2: // Mouth/speech
        arc(x, y, size, size * 0.6, 0, PI);
        // Sound waves
        for (let w = 1; w <= 3; w++) {
          arc(x + size / 2 + w * 5 * scaleFactor, y, w * 6 * scaleFactor, w * 8 * scaleFactor, -QUARTER_PI, QUARTER_PI);
        }
        break;
      case 3: // Footsteps
        // Two feet
        ellipse(x - size / 4, y, size * 0.3, size * 0.6);
        ellipse(x + size / 4, y - size / 4, size * 0.3, size * 0.6);
        break;
      case 4: // Stop symbol
        rect(x - size / 2, y - size / 2, size, size);
        line(x - size / 3, y - size / 3, x + size / 3, y + size / 3);
      ...
      case 5: // Breath mark
        strokeWeight(1.5 * scaleFactor);
        beginShape();
        vertex(x - size / 3, y + size / 4);
        bezierVertex(x - size / 3, y - size / 3, x + size / 3, y - size / 3, x + size / 3, y + size / 4);
        endShape();
        break;
      case 6: // Clap hands
        // Two hands meeting
        arc(x - size / 4, y, size / 2, size / 2, -HALF_PI, HALF_PI);
        arc(x + size / 4, y, size / 2, size / 2, HALF_PI, PI + HALF_PI);
        // Impact lines
        for (let i = 0; i < 3; i++) {
          const ia = rnd(-0.5, 0.5);
          line(x, y + ia * size / 2, x + rnd(-3, 3) * scaleFactor, y + ia * size / 2 - 5 * scaleFactor);
        }
        break;
      case 7: // Silence gesture
        // Finger to lips
        line(x, y - size / 2, x, y + size / 4);
        arc(x, y + size / 3, size / 2, size / 3, 0, PI);
        break;
    }
  }
}

function drawTheaterObjectNotation(voice, section) {
  // Notation for objects/props used in performance - distribute across section
  const objects = ["chair", "table", "book", "glass", "bell", "paper", "hat", "cane"];
  const numObjects = Math.max(2, Math.floor(rnd(2, 5) * features.densityValue));

  for (let o = 0; o < numObjects; o++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 25 + ((o + 0.5) / numObjects) * (section.width - 65) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 20 + ((o % 3) / 2) * (voice.height - 40);
    const y = yBase + rnd(-10, 10) * scaleFactor;
    const size = rnd(18, 30) * scaleFactor;
    const objectType = rndInt(0, 5);
    ...
    switch (objectType) {
      case 0: // Chair
        ...
        // Back
        line(x - size / 3, y, x - size / 3, y - size / 2);
        line(x - size / 3, y - size / 2, x - size / 4, y - size / 2);
        // Legs
        line(x - size / 3, y, x - size / 3, y + size / 3);
        line(x + size / 3, y, x + size / 3, y + size / 3);
        break;
      case 1: // Table
        line(x - size / 2, y, x + size / 2, y);
        ...
        break;
      case 2: // Book
        rect(x - size / 3, y - size / 4, size * 0.6, size * 0.4);
        line(x, y - size / 4, x, y + size / 6);
        break;
      case 3: // Glass
        beginShape();
        vertex(x - size / 4, y - size / 3);
        ...
        break;
      case 4: // Bell
        arc(x, y + size / 6, size * 0.6, size * 0.5, 0, PI);
        ellipse(x, y - size / 4, size * 0.2, size * 0.2);
        break;
      case 5: // Paper
        beginShape();
        ...
        endShape(CLOSE);
        // Fold corner
        line(x + size / 4, y - size / 3, x + size / 4, y - size / 4);
    ...
    textSize(7 * scaleFactor);
    textAlign(CENTER, TOP);
    text(objects[objectType], x, y + size / 2 + 2 * scaleFactor);
  }
}

function drawTheaterMovementArrows(voice, section) {
  // Arrows showing performer movement
  const numArrows = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let a = 0; a < numArrows; a++) {
    const startX = rnd(section.xStart + 20, section.xEnd - 60);
    const startY = rnd(voice.yStart + 15, voice.yEnd - 15);
    const length = rnd(40, 80) * scaleFactor;
    const angle = rnd(-0.5, 0.5);

    const endX = startX + cos(angle) * length;
    const endY = startY + sin(angle) * length;
    ...
    const cy = (startY + endY) / 2 - 15 * scaleFactor;
    beginShape();
    vertex(startX, startY);
    ...
    // Arrowhead
    fill(features.palette.accent);
    const arrowAngle = atan2(endY - cy, endX - cx);
    push();
    translate(endX, endY);
    rotate(arrowAngle);
    ...
    // Start marker (performer)
    noFill();
    stroke(features.palette.accent);
    ellipse(startX, startY, 8 * scaleFactor, 8 * scaleFactor);
  }
}

function drawTheaterSpeechBubbles(voice, section) {
  // Comic-style speech bubbles with text
  const exclamations = ["!", "?", "...", "ah!", "oh", "hmm", "shhh", "ha!", "*", "~"];
  const numBubbles = Math.max(1, Math.floor(rnd(1, 4) * features.densityValue));

  for (let b = 0; b < numBubbles; b++) {
    const x = rnd(section.xStart + 30, section.xEnd - 40);
    const y = rnd(voice.yStart + 20, voice.yEnd - 20);
    const w = rnd(25, 45) * scaleFactor;
    const h = rnd(18, 30) * scaleFactor;
    ...
    beginShape();
    vertex(x - w / 4, y + h / 3);
    vertex(x - w / 3, y + h / 2 + 8 * scaleFactor);
    vertex(x, y + h / 3);
    endShape();
    ...
    textSize(10 * scaleFactor);
    textAlign(CENTER, CENTER);
    text(exclamations[rndInt(0, exclamations.length - 1)], x, y);
  }
}

function drawTheaterTimedActions(voice, section) {
  // Actions with time indicators
  const numActions = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let a = 0; a < numActions; a++) {
    const x = rnd(section.xStart + 20, section.xEnd - 50);
    const y = rnd(voice.yStart + 15, voice.yEnd - 15);
    const duration = rndInt(2, 15);
    ...
    // Duration bar
    const barWidth = duration * 4 * scaleFactor;
    rect(x, y - 5 * scaleFactor, barWidth, 10 * scaleFactor);

    // Fill to show progress point
    fill(features.palette.ink + "30");
    noStroke();
    rect(x, y - 5 * scaleFactor, barWidth * rnd(0.3, 0.8), 10 * scaleFactor);
    ...
    textSize(7 * scaleFactor);
    textAlign(CENTER, TOP);
    text(duration + '"', x + barWidth / 2, y + 8 * scaleFactor);

    // Action marker at start
    stroke(features.palette.accent);
    strokeWeight(2 * scaleFactor);
    line(x, y - 8 * scaleFactor, x, y + 8 * scaleFactor);
  }
...
function drawTheaterSpatialPositions(voice, section) {
  // Stage positions marked with circles/labels - distribute across section
  const positions = ["CS", "SL", "SR", "US", "DS", "USL", "USR", "DSL", "DSR"];
  const numPositions = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));

  for (let p = 0; p < numPositions; p++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 25 + ((p + 0.5) / numPositions) * (section.width - 50) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 20 + ((p % 3) / 2) * (voice.height - 40);
    const y = yBase + rnd(-10, 10) * scaleFactor;
    const size = rnd(20, 35) * scaleFactor;
    ...
    textSize(8 * scaleFactor);
    textAlign(CENTER, CENTER);
    text(positions[rndInt(0, positions.length - 1)], x, y + size / 2 + 8 * scaleFactor);
  }
}

function drawTheaterGestureLines(voice, section) {
  // Flowing lines indicating gestural movements
  const numGestures = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));
  ...
  for (let g = 0; g < numGestures; g++) {
    ...
    for (let p = 0; p < numPoints; p++) {
      const px = prevX + rnd(15, 30) * scaleFactor;
      const py = prevY + rnd(-20, 20) * scaleFactor;
      curveVertex(px, py);
      prevX = px;
      prevY = py;
    }
    ...
    // Endpoint flourish
    stroke(features.palette.accent);
    ellipse(prevX, prevY, 6 * scaleFactor, 6 * scaleFactor);
  }
}

function drawTheaterPropsSymbols(voice, section) {
  // Abstract symbols for props
  const numProps = Math.max(2, Math.floor(rnd(3, 7) * features.densityValue));
  ...
  for (let p = 0; p < numProps; p++) {
    ...
    const propType = rndInt(0, 5);

    switch (propType) {
      case 0: // Circle with X
        ellipse(x, y, size, size);
        line(x - size / 3, y - size / 3, x + size / 3, y + size / 3);
        line(x + size / 3, y - size / 3, x - size / 3, y + size / 3);
        break;
      case 1: // Triangle
        triangle(x, y - size / 2, x - size / 2, y + size / 3, x + size / 2, y + size / 3);
        break;
      case 2: // Diamond
        beginShape();
        vertex(x, y - size / 2);
      ...
      case 3: // Star
        for (let s = 0; s < 5; s++) {
          const angle1 = s * TWO_PI / 5 - HALF_PI;
          const angle2 = (s + 0.5) * TWO_PI / 5 - HALF_PI;
          line(
            x + cos(angle1) * size / 2, y + sin(angle1) * size / 2,
            x + cos(angle2) * size / 4, y + sin(angle2) * size / 4
          );
          line(
            x + cos(angle2) * size / 4, y + sin(angle2) * size / 4,
            x + cos(angle1 + TWO_PI / 5) * size / 2, y + sin(angle1 + TWO_PI / 5) * size / 2
          );
        }
        break;
      case 4: // Plus
...
function drawTheaterEntranceExits(voice, section) {
  // Entrance and exit markers
  const numMarkers = Math.max(1, Math.floor(rnd(2, 4) * features.densityValue));

  for (let m = 0; m < numMarkers; m++) {
    const x = rnd(section.xStart + 30, section.xEnd - 30);
    const y = rnd(voice.yStart + 20, voice.yEnd - 20);
    const size = rnd(15, 25) * scaleFactor;
    const isEntrance = rndBool(0.5);
    ...
    // Arrow (in for entrance, out for exit)
    if (isEntrance) {
      // Arrow pointing in
      line(x - size, y, x - size / 4, y);
      triangle(x - size / 4, y, x - size / 2, y - 4 * scaleFactor, x - size / 2, y + 4 * scaleFactor);
    } else {
      // Arrow pointing out
      line(x + size / 4, y, x + size, y);
      fill(features.palette.accent);
      triangle(x + size, y, x + size * 0.75, y - 4 * scaleFactor, x + size * 0.75, y + 4 * scaleFactor);
    }
    ...
    textSize(7 * scaleFactor);
    textAlign(CENTER, TOP);
    text(isEntrance ? "ENT" : "EXIT", x, y + size / 2 + 3 * scaleFactor);
  }
}

function drawTheaterLightingCues(voice, section) {
  // Lighting cue markers - distribute across section
  const cues = ["LX1", "LX2", "LX3", "FADE", "SPOT", "DIM", "BLACK", "UP"];
  const numCues = Math.max(2, Math.floor(rnd(2, 5) * features.densityValue));

  for (let c = 0; c < numCues; c++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 20 + ((c + 0.5) / numCues) * (section.width - 50) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 15 + ((c % 3) / 2) * (voice.height - 30);
    const y = yBase + rnd(-8, 8) * scaleFactor;
    const size = rnd(18, 28) * scaleFactor;
    ...
    // Light bulb symbol
    ellipse(x, y - size / 4, size * 0.6, size * 0.6);
    // Base
    rect(x - size / 6, y + size / 8, size / 3, size / 4);
    // Rays
    for (let r = 0; r < 6; r++) {
      const angle = r * PI / 3 - HALF_PI;
      const rayStart = size / 3;
      const rayEnd = size / 2;
      line(
        x + cos(angle) * rayStart, y - size / 4 + sin(angle) * rayStart,
        x + cos(angle) * rayEnd, y - size / 4 + sin(angle) * rayEnd
      );
    }
    ...
    textSize(7 * scaleFactor);
    textAlign(CENTER, TOP);
    text(cues[rndInt(0, cues.length - 1)], x, y + size / 2);
  }
}

function drawTheaterAbsurdistMarks(voice, section) {
  // Absurdist/Dada-inspired marks - distribute across section
  const numMarks = Math.max(3, Math.floor(rnd(4, 8) * features.densityValue));

  for (let m = 0; m < numMarks; m++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 15 + ((m + 0.5) / numMarks) * (section.width - 30) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 12 + ((m % 3) / 2) * (voice.height - 24);
    const y = yBase + rnd(-8, 8) * scaleFactor;
    const size = rnd(10, 22) * scaleFactor;
    const markType = rndInt(0, 5);

    stroke(features.palette.ink);
    strokeWeight(rnd(0.8, 2) * scaleFactor);
    noFill();
    ...
    switch (markType) {
      case 0: // Scribble
        beginShape();
        for (let s = 0; s < 8; s++) {
          vertex(x + rnd(-size / 2, size / 2), y + rnd(-size / 2, size / 2));
        }
        endShape();
        break;
      case 1: // Crossed out circle
        ellipse(x, y, size, size);
        strokeWeight(2 * scaleFactor);
        line(x - size / 2, y - size / 2, x + size / 2, y + size / 2);
        break;
      case 2: // Question marks scattered
        fill(features.palette.ink);
        textSize(size);
        textAlign(CENTER, CENTER);
        text("?", x, y);
        text("?", x + rnd(-10, 10) * scaleFactor, y + rnd(-10, 10) * scaleFactor);
        noFill();
        break;
      case 3: // Spiral
        beginShape();
      ...
      case 4: // Zigzag
        beginShape();
        for (let z = 0; z < 5; z++) {
          vertex(x - size / 2 + z * size / 4, y + (z % 2 === 0 ? -size / 4 : size / 4));
        }
        endShape();
        break;
      case 5: // Random dashes
        for (let d = 0; d < 4; d++) {
          const dx = x + rnd(-size / 2, size / 2);
          const dy = y + rnd(-size / 2, size / 2);
          line(dx, dy, dx + rnd(-5, 5) * scaleFactor, dy + rnd(-5, 5) * scaleFactor);
        }
        break;
    }
  }
...
function drawTheaterSynchronizationLines(voice, section) {
  // Vertical lines showing synchronization points
  const numLines = Math.max(2, Math.floor(rnd(3, 7) * features.densityValue));
  ...
  for (let l = 0; l < numLines; l++) {
    ...
    stroke(features.palette.ink + "60");
    strokeWeight(0.8 * scaleFactor);
    setLineDash([4 * scaleFactor, 4 * scaleFactor]);
    line(x, voice.yStart, x, voice.yEnd);
    setLineDash([]);
    ...
    strokeWeight(1.5 * scaleFactor);
    fill(features.palette.paper);
    ellipse(x, voice.yCenter, 10 * scaleFactor, 10 * scaleFactor);
    ...
    textSize(7 * scaleFactor);
    textAlign(CENTER, CENTER);
    text(l + 1, x, voice.yCenter);
  }
}

function drawTheaterPerformerPaths(voice, section) {
  // Paths showing performer movement across stage
  const numPaths = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));
  ...
  for (let p = 0; p < numPaths; p++) {
    ...
    stroke(features.palette.ink);
    strokeWeight(1 * scaleFactor);
    setLineDash([2 * scaleFactor, 3 * scaleFactor]);
    ...
    for (let s = 0; s < rndInt(4, 7); s++) {
      px += rnd(15, 30) * scaleFactor;
      py += rnd(-15, 15) * scaleFactor;
      vertex(px, py);
    }
    endShape();
    ...
    fill(features.palette.ink + "30");
    for (let f = 0; f < 3; f++) {
      const fx = startX + f * 30 * scaleFactor;
      const fy = y + rnd(-5, 5) * scaleFactor;
      ellipse(fx, fy, 4 * scaleFactor, 6 * scaleFactor);
    }
  }
}

function drawTheaterAudienceMarks(voice, section) {
  // Marks indicating audience awareness/interaction
  const numMarks = Math.max(1, Math.floor(rnd(1, 4) * features.densityValue));

  for (let m = 0; m < numMarks; m++) {
    const x = rnd(section.xStart + 25, section.xEnd - 35);
    const y = rnd(voice.yStart + 15, voice.yEnd - 15);
    const size = rnd(20, 35) * scaleFactor;
    ...
    // Arrow pointing "out" toward audience
    line(x, y, x, y + size);
    // Arrowhead
    fill(features.palette.ink);
    triangle(x, y + size, x - 5 * scaleFactor, y + size - 8 * scaleFactor, x + 5 * scaleFactor, y + size - 8 * scaleFactor);

    // Eye symbol at top
    noFill();
    ellipse(x, y - 5 * scaleFactor, size / 2, size / 4);
    fill(features.palette.ink);
    ellipse(x, y - 5 * scaleFactor, size / 6, size / 6);
    ...
    textSize(6 * scaleFactor);
    textAlign(CENTER, TOP);
    text("→ AUD", x, y + size + 3 * scaleFactor);
  }
}

function drawTheaterDurationBars(voice, section) {
  // Horizontal bars showing action duration
  const numBars = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let b = 0; b < numBars; b++) {
    const x = rnd(section.xStart + 15, section.xEnd - 60);
    const y = rnd(voice.yStart + 10, voice.yEnd - 10);
    const width = rnd(40, 80) * scaleFactor;
    const height = 6 * scaleFactor;
    ...
    strokeWeight(0.5 * scaleFactor);
    for (let t = 0; t <= 4; t++) {
      const tx = x + (t / 4) * width;
      line(tx, y, tx, y + height);
    }
    ...
    const actions = ["hold", "move", "speak", "wait", "act"];
    text(actions[rndInt(0, actions.length - 1)], x, y - 2 * scaleFactor);
  }
}

// ============================================================
// CUES MODE (Christian Wolff) - v3.26.0
// Christian Wolff developed participatory scores where performers
// cue each other, creating interdependent musical relationships.
// His notation emphasizes listening, response, and coordination.
// ============================================================

function drawCuesCueSymbols(voice, section) {
  // Cue symbols - circles with numbers/letters indicating who cues whom
  const numCues = Math.max(3, Math.floor(rnd(5, 10) * features.densityValue));

  for (let c = 0; c < numCues; c++) {
    // Distribute evenly across section with some jitter
    const x = section.xStart + 25 + ((c + 0.5) / numCues) * (section.width - 50) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 15 + ((c % 3) / 2) * (voice.height - 30);
    const y = yBase + rnd(-8, 8) * scaleFactor;
    const size = rnd(18, 30) * scaleFactor;
    const cueType = rndInt(0, 3);
    ...
    if (cueType === 0) {
      ...
      // Letter cue
      text(String.fromCharCode(65 + rndInt(0, 5)), x, y);
    } else if (cueType === 2) {
      // Arrow in circle
      stroke(features.palette.accent);
      strokeWeight(1.5 * scaleFactor);
      const arrowDir = rndInt(0, 3) * HALF_PI;
      line(x, y, x + cos(arrowDir) * size / 3, y + sin(arrowDir) * size / 3);
      // Arrowhead
      fill(features.palette.accent);
      push();
      translate(x + cos(arrowDir) * size / 3, y + sin(arrowDir) * size / 3);
      rotate(arrowDir);
      triangle(0, 0, -5 * scaleFactor, -3 * scaleFactor, -5 * scaleFactor, 3 * scaleFactor);
      pop();
    } else {
      // "CUE" text
      textSize(7 * scaleFactor);
...
function drawCuesConditionalBrackets(voice, section) {
  // Brackets indicating conditional sections (if/then/else) - distribute across section
  const numBrackets = Math.max(2, Math.floor(rnd(1, 4) * features.densityValue));

  for (let b = 0; b < numBrackets; b++) {
    // Distribute evenly across section with jitter
    const x = section.xStart + 20 + ((b + 0.5) / numBrackets) * (section.width - 80) + rnd(-10, 10) * scaleFactor;
    // Alternate vertical position with jitter
    const yBase = voice.yStart + 10 + ((b % 3) / 2) * (voice.height - 30);
    const y = yBase + rnd(-8, 8) * scaleFactor;
    const width = rnd(40, 70) * scaleFactor;
    const height = rnd(20, 35) * scaleFactor;
    ...
    line(x, y, x + 8 * scaleFactor, y);
    line(x, y, x, y + height);
    line(x, y + height, x + 8 * scaleFactor, y + height);

    // Closing bracket
    line(x + width - 8 * scaleFactor, y, x + width, y);
    line(x + width, y, x + width, y + height);
    line(x + width - 8 * scaleFactor, y + height, x + width, y + height);
    ...
    textSize(7 * scaleFactor);
    textAlign(LEFT, TOP);
    const conditions = ["if heard", "when ready", "after cue", "until signal"];
    text(conditions[rndInt(0, conditions.length - 1)], x + 10 * scaleFactor, y + 2 * scaleFactor);
  }
}

function drawCuesResponseMarkers(voice, section) {
  // Markers indicating response to another player's action
  const numMarkers = Math.max(2, Math.floor(rnd(3, 7) * features.densityValue));
  ...
  for (let m = 0; m < numMarkers; m++) {
    ...
    // Response symbol (ear-like or arrow)
    if (rndBool(0.5)) {
      // Curved response arrow
      arc(x, y, size, size, -QUARTER_PI, QUARTER_PI + HALF_PI);
      // Arrow point
      fill(features.palette.accent);
      const endAngle = QUARTER_PI + HALF_PI;
      triangle(
        x + cos(endAngle) * size / 2, y + sin(endAngle) * size / 2,
        x + cos(endAngle) * size / 2 - 4 * scaleFactor, y + sin(endAngle) * size / 2 - 4 * scaleFactor,
        x + cos(endAngle) * size / 2 + 2 * scaleFactor, y + sin(endAngle) * size / 2 - 4 * scaleFactor
      );
    } else {
      // "R" in circle for response
      ellipse(x, y, size, size);
      noStroke();
      fill(features.palette.accent);
      textSize(10 * scaleFactor);
...
function drawCuesListeningIndicators(voice, section) {
  // Ear symbols indicating active listening
  const numEars = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let e = 0; e < numEars; e++) {
    const x = rnd(section.xStart + 20, section.xEnd - 20);
    const y = rnd(voice.yStart + 15, voice.yEnd - 15);
    const size = rnd(15, 25) * scaleFactor;
    ...
    // Ear shape
    arc(x, y, size, size * 1.2, -HALF_PI - 0.3, HALF_PI + 0.3);
    // Inner ear
    arc(x + 3 * scaleFactor, y, size * 0.5, size * 0.6, -HALF_PI, HALF_PI);

    // Sound waves coming in
    stroke(features.palette.ink + "60");
    for (let w = 1; w <= 3; w++) {
      arc(x - size / 2 - w * 5 * scaleFactor, y, w * 8 * scaleFactor, w * 10 * scaleFactor, -QUARTER_PI, QUARTER_PI);
    }
  }
}

function drawCuesWaitSigns(voice, section) {
  // Signs indicating to wait for something
  const numSigns = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let s = 0; s < numSigns; s++) {
    const x = rnd(section.xStart + 20, section.xEnd - 25);
    const y = rnd(voice.yStart + 15, voice.yEnd - 15);
    const size = rnd(14, 24) * scaleFactor;
    ...
    if (rndBool(0.5)) {
      // Hand symbol (stop/wait)
      // Palm
      ellipse(x, y + size / 4, size * 0.7, size * 0.5);
      // Fingers
      for (let f = 0; f < 5; f++) {
        const fx = x - size / 3 + f * size / 6;
        line(fx, y + size / 8, fx, y - size / 3);
      }
    } else {
      // Clock/time symbol
      ellipse(x, y, size, size);
      // Clock hands
      line(x, y, x, y - size / 3);
      line(x, y, x + size / 4, y + size / 6);
      // "WAIT" text below
      noStroke();
      fill(features.palette.ink);
      textSize(6 * scaleFactor);
      textAlign(CENTER, TOP);
      text("WAIT", x, y + size / 2 + 2 * scaleFactor);
    }
  }
}

function drawCuesCoordinationLines(voice, section) {
  // Lines connecting coordinated events
  const numLines = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let l = 0; l < numLines; l++) {
    const x1 = rnd(section.xStart + 25, section.xEnd - 80);
    const y1 = rnd(voice.yStart + 15, voice.yEnd - 15);
    const x2 = x1 + rnd(50, 100) * scaleFactor;
    const y2 = y1 + rnd(-20, 20) * scaleFactor;
    ...
    stroke(features.palette.accent);
    strokeWeight(1 * scaleFactor);
    setLineDash([3 * scaleFactor, 3 * scaleFactor]);

    // Curved coordination line
    noFill();
    const cx = (x1 + x2) / 2;
    const cy = Math.min(y1, y2) - 15 * scaleFactor;
    beginShape();
    vertex(x1, y1);
    quadraticVertex(cx, cy, x2, y2);
    endShape();
...
function drawCuesPlayerNumbers(voice, section) {
  // Numbers indicating which player should act
  const numPlayers = Math.max(2, Math.floor(rnd(3, 7) * features.densityValue));

  for (let p = 0; p < numPlayers; p++) {
    const x = rnd(section.xStart + 15, section.xEnd - 15);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    const size = rnd(12, 20) * scaleFactor;
    const playerNum = rndInt(1, 6);

    stroke(features.palette.ink);
    strokeWeight(1 * scaleFactor);
    noFill();

    // Square around number
    rect(x - size / 2, y - size / 2, size, size);
...
function drawCuesReactionArrows(voice, section) {
  // Arrows showing reaction/response direction
  const numArrows = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let a = 0; a < numArrows; a++) {
    const x = rnd(section.xStart + 30, section.xEnd - 40);
    const y = rnd(voice.yStart + 15, voice.yEnd - 15);
    const length = rnd(30, 50) * scaleFactor;
    const angle = rnd(-0.3, 0.3);
    ...
    textSize(6 * scaleFactor);
    textAlign(CENTER, BOTTOM);
    text("react", (x + endX) / 2, Math.min(y, endY) - 3 * scaleFactor);
  }
}

function drawCuesHoldSymbols(voice, section) {
  // Symbols indicating to hold/sustain
  const numHolds = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let h = 0; h < numHolds; h++) {
    const x = rnd(section.xStart + 20, section.xEnd - 40);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    const width = rnd(30, 60) * scaleFactor;

    stroke(features.palette.ink);
    strokeWeight(1.5 * scaleFactor);
    noFill();

    // Fermata-like hold symbol
    arc(x + width / 2, y, width, width * 0.6, PI, TWO_PI);
    fill(features.palette.ink);
    ellipse(x + width / 2, y - 5 * scaleFactor, 4 * scaleFactor, 4 * scaleFactor);

    // Duration line
    stroke(features.palette.ink + "60");
    strokeWeight(2 * scaleFactor);
    line(x, y + 8 * scaleFactor, x + width, y + 8 * scaleFactor);
  }
}

function drawCuesReleaseMarks(voice, section) {
  // Marks indicating release points
  const numMarks = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));
  ...
  for (let m = 0; m < numMarks; m++) {
    ...
    // Release symbol - open hand or asterisk
    if (rndBool(0.5)) {
      // Asterisk release
      for (let r = 0; r < 6; r++) {
        const angle = r * PI / 3;
      ...
      line(x - size / 2, y - size / 2, x - size / 2, y + size / 2);
      line(x - size / 2, y + size / 2, x + size / 2, y + size / 2);
      // Arrow pointing right (release)
      line(x - size / 4, y, x + size / 2, y);
      fill(features.palette.accent);
      triangle(x + size / 2, y, x + size / 4, y - 3 * scaleFactor, x + size / 4, y + 3 * scaleFactor);
    }
  }
}

function drawCuesAttentionPoints(voice, section) {
  // Points requiring special attention
  const numPoints = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));
  ...
  for (let p = 0; p < numPoints; p++) {
    ...
    noStroke();
    fill(features.palette.accent);
    rect(x - 2 * scaleFactor, y - size / 4, 4 * scaleFactor, size / 3);
    ellipse(x, y + size / 4 - 2 * scaleFactor, 4 * scaleFactor, 4 * scaleFactor);
  }
}

function drawCuesFlexibleDurations(voice, section) {
  // Wavy lines indicating flexible duration
  const numDurations = Math.max(1, Math.floor(rnd(2, 4) * features.densityValue));

  for (let d = 0; d < numDurations; d++) {
    const startX = rnd(section.xStart + 15, section.xEnd - 60);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    const length = rnd(40, 80) * scaleFactor;
    ...
    // Wavy duration line
    beginShape();
    for (let x = startX; x <= startX + length; x += 4 * scaleFactor) {
      const waveY = y + sin((x - startX) * 0.15) * 4 * scaleFactor;
      vertex(x, waveY);
    }
    endShape();

    // End brackets
    line(startX, y - 6 * scaleFactor, startX, y + 6 * scaleFactor);
    line(startX + length, y - 6 * scaleFactor, startX + length, y + 6 * scaleFactor);
    ...
    textSize(6 * scaleFactor);
    textAlign(CENTER, TOP);
    text("flex", startX + length / 2, y + 8 * scaleFactor);
  }
}

function drawCuesMuteSymbols(voice, section) {
  // Symbols indicating muting/silence
  const numMutes = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let m = 0; m < numMutes; m++) {
    const x = rnd(section.xStart + 20, section.xEnd - 20);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    const size = rnd(14, 22) * scaleFactor;
...
function drawCuesPlaySymbols(voice, section) {
  // Triangle play symbols
  const numPlay = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let p = 0; p < numPlay; p++) {
    const x = rnd(section.xStart + 20, section.xEnd - 20);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    const size = rnd(12, 20) * scaleFactor;
    ...
    stroke(features.palette.accent);
    strokeWeight(1.2 * scaleFactor);
    fill(features.palette.accent + "30");

    // Play triangle
    triangle(x - size / 3, y - size / 2, x - size / 3, y + size / 2, x + size / 2, y);
  }
}

function drawCuesPauseIndicators(voice, section) {
  // Pause symbols
  const numPauses = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));
...
function drawCuesGroupBrackets(voice, section) {
  // Brackets grouping players together
  const numGroups = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));

  for (let g = 0; g < numGroups; g++) {
    const x = rnd(section.xStart + 20, section.xEnd - 50);
    const y = voice.yStart + 5 * scaleFactor;
    const width = rnd(35, 60) * scaleFactor;
    const height = voice.yEnd - voice.yStart - 10 * scaleFactor;
    ...
    // Left curly bracket
    beginShape();
    vertex(x + 8 * scaleFactor, y);
    quadraticVertex(x, y, x, y + height / 4);
    vertex(x, y + height / 2 - 5 * scaleFactor);
    vertex(x - 5 * scaleFactor, y + height / 2);
    vertex(x, y + height / 2 + 5 * scaleFactor);
    vertex(x, y + height * 3 / 4);
    quadraticVertex(x, y + height, x + 8 * scaleFactor, y + height);
    endShape();
    ...
    textSize(8 * scaleFactor);
    textAlign(CENTER, CENTER);
    const groupLabels = ["I", "II", "III", "A", "B"];
    text(groupLabels[rndInt(0, groupLabels.length - 1)], x - 12 * scaleFactor, y + height / 2);

    // Player numbers inside
    stroke(features.palette.ink + "60");
    strokeWeight(0.8 * scaleFactor);
    for (let p = 0; p < rndInt(2, 4); p++) {
      const px = x + 15 * scaleFactor + p * 15 * scaleFactor;
      const py = y + height / 2;
      noFill();
      rect(px - 6 * scaleFactor, py - 6 * scaleFactor, 12 * scaleFactor, 12 * scaleFactor);
      noStroke();
      fill(features.palette.ink + "80");
      textSize(8 * scaleFactor);
      text(p + 1, px, py);
...
// Raven Chacon is a Diné (Navajo) composer who creates flag-scores
// and land-based notations drawing on Indigenous perspectives
// on sound, space, and environment.
// ============================================================

function drawIndigenousFlagSymbols(voice, section) {
  // Flags as notation elements (inspired by Chacon's flag scores)
  const numFlags = Math.max(2, Math.floor(rnd(3, 7) * features.densityValue));

  for (let f = 0; f < numFlags; f++) {
    const x = rnd(section.xStart + 25, section.xEnd - 25);
    const y = rnd(voice.yStart + 25, voice.yEnd - 15);
    const poleHeight = rnd(30, 50) * scaleFactor;
    const flagWidth = rnd(20, 35) * scaleFactor;
    const flagHeight = rnd(15, 25) * scaleFactor;

    stroke(features.palette.ink);
    strokeWeight(1.5 * scaleFactor);

    // Pole
    line(x, y, x, y - poleHeight);

    // Flag (waving)
    noFill();
    beginShape();
    vertex(x, y - poleHeight);
    bezierVertex(
      x + flagWidth / 3, y - poleHeight - flagHeight / 4,
      x + flagWidth * 2 / 3, y - poleHeight + flagHeight / 4,
      x + flagWidth, y - poleHeight
    );
    vertex(x + flagWidth, y - poleHeight + flagHeight);
    bezierVertex(
      x + flagWidth * 2 / 3, y - poleHeight + flagHeight - flagHeight / 4,
      x + flagWidth / 3, y - poleHeight + flagHeight + flagHeight / 4,
...
function drawIndigenousLandMarkers(voice, section) {
  // Markers representing places on the land
  const numMarkers = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));
  ...
  for (let m = 0; m < numMarkers; m++) {
    ...
    beginShape();
    vertex(x, y - size);
    bezierVertex(x + size / 2, y - size, x + size / 2, y - size / 3, x, y);
    bezierVertex(x - size / 2, y - size / 3, x - size / 2, y - size, x, y - size);
    endShape(CLOSE);

    // Inner dot
    fill(features.palette.accent);
    ellipse(x, y - size * 0.6, size / 3, size / 3);
  }
}

function drawIndigenousWindDirections(voice, section) {
  // Wind direction indicators
  const numWinds = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let w = 0; w < numWinds; w++) {
    const x = rnd(section.xStart + 30, section.xEnd - 40);
    const y = rnd(voice.yStart + 15, voice.yEnd - 15);
    const length = rnd(30, 50) * scaleFactor;
    const angle = rnd(0, TWO_PI);
    ...
    // Wind lines (multiple wavy lines)
    for (let l = 0; l < 3; l++) {
      const ly = y + (l - 1) * 4 * scaleFactor;
      beginShape();
      for (let i = 0; i <= 6; i++) {
        const wx = x + cos(angle) * (i / 6) * length;
        const wy = ly + sin(angle) * (i / 6) * length + sin(i * 1.5) * 3 * scaleFactor;
        vertex(wx, wy);
      }
      endShape();
    }
    ...
    translate(endX, endY);
    rotate(angle);
    triangle(0, 0, -6 * scaleFactor, -3 * scaleFactor, -6 * scaleFactor, 3 * scaleFactor);
    pop();
  }
}

function drawIndigenousHorizonLines(voice, section) {
  // Horizon line with landscape elements
  const y = voice.yCenter;

  stroke(features.palette.ink);
  strokeWeight(1 * scaleFactor);

  // Main horizon
  line(section.xStart + 10, y, section.xEnd - 10, y);

  // Landscape features above
  noFill();
  const numFeatures = rndInt(3, 6);
  for (let f = 0; f < numFeatures; f++) {
    const fx = section.xStart + 20 + f * (section.xEnd - section.xStart - 40) / numFeatures;
    const fHeight = rnd(15, 35) * scaleFactor;
    const fWidth = rnd(20, 40) * scaleFactor;

    // Mountain/mesa shape
    beginShape();
    vertex(fx - fWidth / 2, y);
    vertex(fx - fWidth / 3, y - fHeight);
    vertex(fx + fWidth / 3, y - fHeight);
    vertex(fx + fWidth / 2, y);
    endShape();
  }
...
function drawIndigenousCelestialMarks(voice, section) {
  // Sun, moon, stars
  const numMarks = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));
  ...
  for (let m = 0; m < numMarks; m++) {
    ...
    if (type === 0) {
      ...
      // Rays
      for (let r = 0; r < 8; r++) {
        const angle = r * QUARTER_PI;
        line(
          x + cos(angle) * size / 2, y + sin(angle) * size / 2,
          x + cos(angle) * size * 0.8, y + sin(angle) * size * 0.8
        );
      }
    } else if (type === 1) {
      // Moon (crescent)
      ...
      fill(features.palette.paper);
      noStroke();
      ellipse(x + size / 4, y, size * 0.7, size * 0.7);
    } else {
      // Star
      fill(features.palette.accent);
      for (let s = 0; s < 4; s++) {
        const angle = s * HALF_PI;
        triangle(
          x, y,
          x + cos(angle - 0.2) * size / 2, y + sin(angle - 0.2) * size / 2,
          x + cos(angle + 0.2) * size / 2, y + sin(angle + 0.2) * size / 2
        );
      }
    }
...
function drawIndigenousWaterSymbols(voice, section) {
  // Water/river representations
  const numWaters = Math.max(1, Math.floor(rnd(1, 4) * features.densityValue));

  for (let w = 0; w < numWaters; w++) {
    const startX = rnd(section.xStart + 15, section.xEnd - 60);
    const y = rnd(voice.yStart + 15, voice.yEnd - 15);
    const length = rnd(40, 70) * scaleFactor;
    ...
    // Wavy water lines
    for (let l = 0; l < 3; l++) {
      const ly = y + l * 5 * scaleFactor;
      beginShape();
      for (let i = 0; i <= 8; i++) {
        vertex(startX + (i / 8) * length, ly + sin(i * 0.8 + l * 0.5) * 4 * scaleFactor);
      }
      endShape();
    }
  }
...
function drawIndigenousFireSymbols(voice, section) {
  // Fire representations
  const numFires = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let f = 0; f < numFires; f++) {
    const x = rnd(section.xStart + 20, section.xEnd - 20);
    const y = rnd(voice.yStart + 20, voice.yEnd - 10);
    const size = rnd(15, 28) * scaleFactor;
    ...
    for (let fl = 0; fl < 3; fl++) {
      const fx = x + (fl - 1) * size / 3;
      const fh = size * rnd(0.6, 1);
      beginShape();
      vertex(fx, y);
      bezierVertex(fx - size / 6, y - fh / 2, fx - size / 8, y - fh, fx, y - fh);
      bezierVertex(fx + size / 8, y - fh, fx + size / 6, y - fh / 2, fx, y);
      endShape();
    }
  }
}

function drawIndigenousEarthSymbols(voice, section) {
  // Earth/ground representations
  const numSymbols = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));
  ...
  for (let s = 0; s < numSymbols; s++) {
    ...
    // Layered earth symbol
    for (let l = 0; l < 4; l++) {
      const lw = size - l * size / 6;
      line(x - lw / 2, y + l * 4 * scaleFactor, x + lw / 2, y + l * 4 * scaleFactor);
    }

    // Small growth marks
    for (let g = 0; g < 2; g++) {
      const gx = x + (g - 0.5) * size / 2;
      line(gx, y, gx, y - size / 3);
      line(gx - 3 * scaleFactor, y - size / 4, gx, y - size / 3);
      line(gx + 3 * scaleFactor, y - size / 4, gx, y - size / 3);
    }
  }
}

function drawIndigenousPathwayLines(voice, section) {
  // Paths/trails
  const numPaths = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));
  ...
  for (let p = 0; p < numPaths; p++) {
    ...
    stroke(features.palette.ink + "70");
    strokeWeight(1 * scaleFactor);
    setLineDash([4 * scaleFactor, 3 * scaleFactor]);

    beginShape();
    let px = startX;
    let py = startY;
    for (let s = 0; s < rndInt(5, 8); s++) {
      vertex(px, py);
      px += rnd(15, 25) * scaleFactor;
      py += rnd(-10, 10) * scaleFactor;
    }
    endShape();

    setLineDash([]);

    // Footprint marks along path
    fill(features.palette.ink + "40");
    noStroke();
    for (let f = 0; f < 3; f++) {
      const fx = startX + f * 25 * scaleFactor;
      const fy = startY + rnd(-3, 3) * scaleFactor;
      ellipse(fx, fy, 3 * scaleFactor, 5 * scaleFactor);
    }
  }
}

function drawIndigenousGatheringCircles(voice, section) {
  // Circles representing gathering places - distribute across section
  const numCircles = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));

  for (let c = 0; c < numCircles; c++) {
    // Distribute evenly across section with some jitter
    const x = section.xStart + ((c + 0.5) / numCircles) * section.width + rnd(-15, 15) * scaleFactor;
    const y = rnd(voice.yStart + 25, voice.yEnd - 25);
    const size = rnd(25, 45) * scaleFactor;
    ...
    // People marks around
    const numPeople = rndInt(4, 8);
    for (let p = 0; p < numPeople; p++) {
      const angle = (p / numPeople) * TWO_PI;
...
function drawIndigenousCardinalPoints(voice, section) {
  // Four directions markers
  const x = (section.xStart + section.xEnd) / 2;
  const y = voice.yCenter;
  const size = Math.min(40, (voice.yEnd - voice.yStart) / 2 - 5) * scaleFactor;
  ...
  textAlign(CENTER, CENTER);
  noStroke();
  text("N", x, y - size - 8 * scaleFactor);
  text("S", x, y + size + 8 * scaleFactor);
  text("E", x + size + 8 * scaleFactor, y);
  text("W", x - size - 8 * scaleFactor, y);
...
function drawIndigenousSeasonalMarks(voice, section) {
  // Seasonal cycle indicators
  const x = rnd(section.xStart + 35, section.xEnd - 35);
  const y = rnd(voice.yStart + 30, voice.yEnd - 30);
  const size = rnd(25, 40) * scaleFactor;
  ...
  // Different marks in each quadrant
  // Spring (top-right) - small shoots
  line(x + size / 6, y - size / 4, x + size / 6, y - size / 6);
  // Summer (bottom-right) - sun
  fill(features.palette.accent);
  ellipse(x + size / 4, y + size / 4, size / 6, size / 6);
  // Fall (bottom-left) - leaf
  noFill();
  arc(x - size / 4, y + size / 4, size / 5, size / 5, 0, PI);
  // Winter (top-left) - snowflake
  for (let i = 0; i < 3; i++) {
    const a = i * PI / 3;
    line(x - size / 4 + cos(a) * size / 10, y - size / 4 + sin(a) * size / 10,
         x - size / 4 - cos(a) * size / 10, y - size / 4 - sin(a) * size / 10);
  }
}

function drawIndigenousSilenceSpaces(voice, section) {
  // Empty/silent spaces marked
  const numSpaces = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));

  for (let s = 0; s < numSpaces; s++) {
    const x = rnd(section.xStart + 20, section.xEnd - 40);
    const y = rnd(voice.yStart + 15, voice.yEnd - 25);
    const w = rnd(30, 50) * scaleFactor;
    const h = rnd(20, 35) * scaleFactor;

    stroke(features.palette.ink + "50");
    strokeWeight(0.8 * scaleFactor);
    setLineDash([2 * scaleFactor, 4 * scaleFactor]);
    noFill();
...
function drawIndigenousVoiceContours(voice, section) {
  // Voice/singing contour lines
  const numContours = Math.max(1, Math.floor(rnd(2, 4) * features.densityValue));

  for (let c = 0; c < numContours; c++) {
    const startX = rnd(section.xStart + 15, section.xEnd - 60);
    const y = rnd(voice.yStart + 15, voice.yEnd - 15);
    const length = rnd(50, 80) * scaleFactor;
    ...
    beginShape();
    for (let i = 0; i <= 12; i++) {
      const vx = startX + (i / 12) * length;
      const vy = y + sin(i * 0.6) * rnd(10, 20) * scaleFactor + rnd(-3, 3) * scaleFactor;
      vertex(vx, vy);
    }
    endShape();
  }
...
function drawIndigenousDrumPatterns(voice, section) {
  // Drum beat patterns
  const numPatterns = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));
  ...
  for (let p = 0; p < numPatterns; p++) {
    const x = rnd(section.xStart + 20, section.xEnd - 60);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    const numBeats = rndInt(4, 8);

    for (let b = 0; b < numBeats; b++) {
      const bx = x + b * 12 * scaleFactor;
      const isStrong = b === 0 || b === 4;

      stroke(features.palette.ink);
      strokeWeight(1 * scaleFactor);
      fill(isStrong ? features.palette.ink : features.palette.paper);
      ellipse(bx, y, isStrong ? 8 * scaleFactor : 5 * scaleFactor, isStrong ? 8 * scaleFactor : 5 * scaleFactor);
    }

    // Drum symbol
    noFill();
    stroke(features.palette.accent);
    ellipse(x - 15 * scaleFactor, y, 12 * scaleFactor, 8 * scaleFactor);
  }
}

function drawIndigenousBreathMarks(voice, section) {
  // Breath/breathing indicators
  const numBreaths = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));

  for (let b = 0; b < numBreaths; b++) {
    const x = rnd(section.xStart + 15, section.xEnd - 15);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    const size = rnd(10, 18) * scaleFactor;
    ...
    // Breath symbol (comma-like or wavy)
    if (rndBool(0.5)) {
      // Comma breath
      arc(x, y, size, size, 0, PI);
      fill(features.palette.ink);
      ellipse(x + size / 2, y, 3 * scaleFactor, 3 * scaleFactor);
    } else {
      // Wavy breath
      beginShape();
      for (let i = 0; i < 4; i++) {
        vertex(x + i * size / 4, y + (i % 2 === 0 ? -1 : 1) * 3 * scaleFactor);
      }
      endShape();
    }
  }
...
// ============================================================
// DEEP LISTENING MODE (Pauline Oliveros) - v3.28.0
// Pauline Oliveros developed Deep Listening practice integrating
// meditation and awareness. Her scores often feature mandala-like
// circular patterns and breath/attention-based instructions.
// ============================================================

function drawDeepListeningMandalaCircles(voice, section) {
  // Mandala-like concentric circles - draw multiple across section
  const numMandalas = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));
  const maxSize = Math.min(45, (voice.yEnd - voice.yStart) / 2 - 5) * scaleFactor;

  for (let m = 0; m < numMandalas; m++) {
    const x = section.xStart + ((m + 0.5) / numMandalas) * section.width;
    const y = voice.yCenter + rnd(-10, 10) * scaleFactor;
    const numRings = rndInt(3, 6);
    ...
    for (let r = 0; r < numRings; r++) {
      const ringSize = (r + 1) / numRings * maxSize * 2;
      strokeWeight((numRings - r) * 0.2 * scaleFactor);
...
function drawDeepListeningBreathCycles(voice, section) {
  // Breath cycle visualizations
  const numCycles = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));

  for (let c = 0; c < numCycles; c++) {
    const x = rnd(section.xStart + 30, section.xEnd - 60);
    const y = rnd(voice.yStart + 20, voice.yEnd - 20);
    const width = rnd(50, 80) * scaleFactor;
    const height = rnd(20, 35) * scaleFactor;
    ...
    beginShape();
    vertex(x, y);
    bezierVertex(x + width / 4, y - height, x + width * 3 / 4, y - height, x + width, y);
    endShape();

    // Exhale curve
    beginShape();
    vertex(x + width, y);
    bezierVertex(x + width * 3 / 4, y + height, x + width / 4, y + height, x, y);
    endShape();
    ...
    textAlign(CENTER, TOP);
    text("out", x + width / 2, y + height / 2);
  }
}

function drawDeepListeningAwarenessFoci(voice, section) {
  // Points of focus/attention
  const numFoci = Math.max(2, Math.floor(rnd(3, 7) * features.densityValue));
  ...
  for (let f = 0; f < numFoci; f++) {
    ...
    // Attention arrows pointing in
    noFill();
    stroke(features.palette.ink + "60");
    for (let a = 0; a < 4; a++) {
      const angle = a * HALF_PI + QUARTER_PI;
      const startR = size / 2 + 10 * scaleFactor;
      const endR = size / 2 + 3 * scaleFactor;
      line(x + cos(angle) * startR, y + sin(angle) * startR,
           x + cos(angle) * endR, y + sin(angle) * endR);
    }
  }
}

function drawDeepListeningConcentricRings(voice, section) {
  // Expanding concentric rings
  const numSets = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));

  for (let s = 0; s < numSets; s++) {
    const x = rnd(section.xStart + 35, section.xEnd - 35);
    const y = rnd(voice.yStart + 30, voice.yEnd - 30);
    const maxSize = rnd(30, 50) * scaleFactor;
    const numRings = rndInt(3, 6);
    ...
    for (let r = 0; r < numRings; r++) {
      const ringSize = (r + 1) / numRings * maxSize * 2;
      const alpha = Math.floor(255 * (1 - r / numRings));
...
function drawDeepListeningMeditationText(voice, section) {
  // Meditation instruction text
  const instructions = [
    "listen", "breathe", "attend", "open", "receive",
    "expand", "soften", "center", "ground", "release"
  ];
  const numTexts = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));

  for (let t = 0; t < numTexts; t++) {
    const x = rnd(section.xStart + 20, section.xEnd - 40);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    ...
    textAlign(LEFT, CENTER);
    textStyle(ITALIC);
    text(instructions[rndInt(0, instructions.length - 1)], x, y);
    textStyle(NORMAL);
  }
}

function drawDeepListeningExpansionArrows(voice, section) {
  ...
  for (let a = 0; a < numArrows; a++) {
    ...
    for (let d = 0; d < 4; d++) {
      ...
      fill(features.palette.accent);
      push();
      translate(x + cos(angle) * endR, y + sin(angle) * endR);
      rotate(angle);
      triangle(0, 0, -5 * scaleFactor, -3 * scaleFactor, -5 * scaleFactor, 3 * scaleFactor);
      pop();
      noFill();
...
function drawDeepListeningContractionArrows(voice, section) {
  // Arrows pointing inward
  const numArrows = Math.max(1, Math.floor(rnd(2, 4) * features.densityValue));
...
function drawDeepListeningCenterPoints(voice, section) {
  // Emphasized center points
  const numPoints = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));
...
function drawDeepListeningRadiatingLines(voice, section) {
  ...
  for (let s = 0; s < numSets; s++) {
    ...
    const y = rnd(voice.yStart + 25, voice.yEnd - 25);
    const innerR = rnd(5, 10) * scaleFactor;
    const outerR = rnd(25, 40) * scaleFactor;
    const numLines = rndInt(8, 16);
    ...
    for (let l = 0; l < numLines; l++) {
      const angle = (l / numLines) * TWO_PI;
      line(x + cos(angle) * innerR, y + sin(angle) * innerR,
           x + cos(angle) * outerR, y + sin(angle) * outerR);
    }

    // Center
    fill(features.palette.ink);
    noStroke();
    ellipse(x, y, innerR, innerR);
  }
}

function drawDeepListeningPulseMarks(voice, section) {
  // Pulse/heartbeat-like marks
  const numPulses = Math.max(1, Math.floor(rnd(2, 4) * features.densityValue));

  for (let p = 0; p < numPulses; p++) {
    const startX = rnd(section.xStart + 15, section.xEnd - 50);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    const width = rnd(40, 60) * scaleFactor;
    ...
    beginShape();
    vertex(startX, y);
    vertex(startX + width * 0.2, y);
    vertex(startX + width * 0.3, y - 10 * scaleFactor);
    vertex(startX + width * 0.4, y + 5 * scaleFactor);
    vertex(startX + width * 0.5, y);
    vertex(startX + width, y);
    endShape();
  }
}

function drawDeepListeningAttentionZones(voice, section) {
  // Zones of attention
  const numZones = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));

  for (let z = 0; z < numZones; z++) {
    const x = rnd(section.xStart + 25, section.xEnd - 45);
    const y = rnd(voice.yStart + 15, voice.yEnd - 25);
    const w = rnd(35, 55) * scaleFactor;
    const h = rnd(25, 40) * scaleFactor;
    ...
    stroke(features.palette.ink + "50");
    strokeWeight(1 * scaleFactor);
    fill(features.palette.accent + "10");

    // Soft rounded zone
    rect(x, y, w, h, 10 * scaleFactor);
    ...
    textSize(7 * scaleFactor);
    textAlign(CENTER, CENTER);
    text("attend", x + w / 2, y + h / 2);
  }
}

function drawDeepListeningSilenceFields(voice, section) {
  // Areas of silence
  const numFields = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));
...
function drawDeepListeningListeningDepths(voice, section) {
  // Depth indicators
  const numDepths = Math.max(1, Math.floor(rnd(2, 4) * features.densityValue));

  for (let d = 0; d < numDepths; d++) {
    const x = rnd(section.xStart + 25, section.xEnd - 25);
    const y = rnd(voice.yStart + 15, voice.yEnd - 30);
    const depth = rnd(25, 40) * scaleFactor;
    ...
    line(x, y, x, y + depth);
    fill(features.palette.ink);
    triangle(x, y + depth, x - 4 * scaleFactor, y + depth - 6 * scaleFactor, x + 4 * scaleFactor, y + depth - 6 * scaleFactor);

    // Depth markers
    noFill();
    for (let m = 0; m < 3; m++) {
      const my = y + (m + 1) * depth / 4;
      line(x - 4 * scaleFactor, my, x + 4 * scaleFactor, my);
    }
  }
}

function drawDeepListeningSonicEnvironment(voice, section) {
  // Environmental sound representation
  const x = (section.xStart + section.xEnd) / 2;
  const y = voice.yCenter;
  const size = Math.min(50, (voice.yEnd - voice.yStart) / 2 - 10) * scaleFactor;
  ...
  for (let r = 1; r <= 4; r++) {
    const ringSize = r * size / 2;
    arc(x, y, ringSize, ringSize, -QUARTER_PI, PI + QUARTER_PI);
  }

  // Center listener
  fill(features.palette.accent);
  noStroke();
  ellipse(x, y, 8 * scaleFactor, 8 * scaleFactor);

  // "environment" label
  fill(features.palette.ink + "60");
  textSize(6 * scaleFactor);
  textAlign(CENTER, TOP);
  text("sonic field", x, y + size / 2 + 5 * scaleFactor);
}

function drawDeepListeningBodyAwareness(voice, section) {
  ...
  for (let m = 0; m < numMarks; m++) {
    ...
    // Simple body outline
    // Head
    ellipse(x, y - size / 3, size / 4, size / 4);
    // Body
    line(x, y - size / 4, x, y + size / 4);
    // Arms
    line(x - size / 4, y - size / 8, x + size / 4, y - size / 8);
    // Legs
    line(x, y + size / 4, x - size / 6, y + size / 2);
    line(x, y + size / 4, x + size / 6, y + size / 2);

    // Awareness marks
    stroke(features.palette.accent + "60");
    for (let a = 0; a < 3; a++) {
      const ay = y - size / 4 + a * size / 4;
      arc(x + size / 3, ay, 10 * scaleFactor, 10 * scaleFactor, -HALF_PI, HALF_PI);
    }
  }
}

function drawDeepListeningGroupConnections(voice, section) {
  // Connections between group members
  const numPeople = rndInt(3, 5);
  const centerX = (section.xStart + section.xEnd) / 2;
  const centerY = voice.yCenter;
  const radius = Math.min(40, (voice.yEnd - voice.yStart) / 2 - 10) * scaleFactor;
  ...
  const positions = [];
  for (let p = 0; p < numPeople; p++) {
    const angle = (p / numPeople) * TWO_PI - HALF_PI;
    positions.push({
      x: centerX + cos(angle) * radius,
      y: centerY + sin(angle) * radius
    });
  ...
  for (const pos of positions) {
    ellipse(pos.x, pos.y, 8 * scaleFactor, 8 * scaleFactor);
  }
}

// ============================================================
// SOUND MAP MODE (Annea Lockwood) - v3.29.0
// Annea Lockwood creates scores based on river maps, geography,
// and environmental sound mapping. Her work explores the sonic
// characteristics of places and waterways.
// ============================================================

function drawSoundMapRiverLines(voice, section) {
  // River/stream flowing lines - span full section width
  const numRivers = Math.max(2, Math.floor(rnd(2, 5) * features.densityValue));

  for (let r = 0; r < numRivers; r++) {
    const startX = section.xStart + 10;
    const startY = rnd(voice.yStart + 15, voice.yEnd - 15);

    stroke(features.palette.accent);
    strokeWeight(rnd(1.5, 3) * scaleFactor);
    noFill();

    // Meandering river - span full section
    beginShape();
    let rx = startX;
    let ry = startY;
    vertex(rx, ry);

    const segmentWidth = (section.xEnd - section.xStart - 20) / 8;
    for (let s = 0; s < 8; s++) {
      rx = section.xStart + 10 + (s + 1) * segmentWidth;
      ry += rnd(-12, 12) * scaleFactor;
      // Keep in bounds
      ry = constrain(ry, voice.yStart + 10, voice.yEnd - 10);
      curveVertex(rx, ry);
    }
    endShape();
  }
...
function drawSoundMapTopographicContours(voice, section) {
  // Topographic contour lines
  const x = rnd(section.xStart + 35, section.xEnd - 35);
  const y = rnd(voice.yStart + 30, voice.yEnd - 30);
  const maxSize = rnd(30, 50) * scaleFactor;
  const numContours = rndInt(4, 7);
  ...
  for (let c = 0; c < numContours; c++) {
    const size = (c + 1) / numContours * maxSize;
    // Irregular contour
    beginShape();
    for (let a = 0; a < TWO_PI; a += 0.3) {
      const r = size + sin(a * 3 + c) * size / 6;
      vertex(x + cos(a) * r, y + sin(a) * r * 0.7);
    }
    endShape(CLOSE);
  }
}

function drawSoundMapLocationMarkers(voice, section) {
  // Sound recording location markers
  const numMarkers = Math.max(2, Math.floor(rnd(3, 7) * features.densityValue));
...
function drawSoundMapSoundSources(voice, section) {
  // Sound source indicators
  const numSources = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));
  ...
  for (let s = 0; s < numSources; s++) {
    ...
    for (let w = 1; w <= 3; w++) {
      arc(x, y, w * size / 2, w * size / 2, -QUARTER_PI, QUARTER_PI);
    }

    // Source point
    fill(features.palette.ink);
    ellipse(x - 2 * scaleFactor, y, 4 * scaleFactor, 4 * scaleFactor);
  }
}

function drawSoundMapFlowArrows(voice, section) {
  // Water/sound flow direction arrows
  const numArrows = Math.max(2, Math.floor(rnd(3, 6) * features.densityValue));

  for (let a = 0; a < numArrows; a++) {
    const x = rnd(section.xStart + 20, section.xEnd - 35);
    const y = rnd(voice.yStart + 12, voice.yEnd - 12);
    const length = rnd(25, 45) * scaleFactor;
    const angle = rnd(-0.3, 0.3);
    ...
    const endX = x + cos(angle) * length;
    const endY = y + sin(angle) * length;
    const cx = x + cos(angle) * length / 2;
    const cy = y + sin(angle) * length / 2 - 8 * scaleFactor;
...
function drawSoundMapConfluencePoints(voice, section) {
  // Points where rivers/sounds meet
  const numPoints = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));
  ...
  for (let p = 0; p < numPoints; p++) {
    ...
    // Two lines meeting
    line(x - 25 * scaleFactor, y - 15 * scaleFactor, x, y);
    line(x - 25 * scaleFactor, y + 15 * scaleFactor, x, y);
    line(x, y, x + 30 * scaleFactor, y);
...
function drawSoundMapWatershedBoundaries(voice, section) {
  // Watershed boundary lines
  const y = voice.yCenter;

  stroke(features.palette.ink + "60");
  strokeWeight(1 * scaleFactor);
  setLineDash([6 * scaleFactor, 3 * scaleFactor]);

  // Ridge line across section
  beginShape();
  for (let x = section.xStart + 10; x <= section.xEnd - 10; x += 15 * scaleFactor) {
    vertex(x, y + rnd(-10, 10) * scaleFactor);
  }
  endShape();
  ...
  textSize(6 * scaleFactor);
  textAlign(CENTER, BOTTOM);
  text("watershed", (section.xStart + section.xEnd) / 2, y - 15 * scaleFactor);
}

function drawSoundMapElevationMarks(voice, section) {
  ...
  for (let m = 0; m < numMarks; m++) {
    ...
    textSize(6 * scaleFactor);
    textAlign(CENTER, TOP);
    text(rndInt(100, 3000), x, y + size / 2 + 2 * scaleFactor);
  }
}

function drawSoundMapFieldRecordingSymbols(voice, section) {
  // Microphone/recording symbols
  const numSymbols = Math.max(1, Math.floor(rnd(2, 5) * features.densityValue));
  ...
  for (let s = 0; s < numSymbols; s++) {
    ...
    // Recording indicator
    fill(features.palette.accent);
    ellipse(x + size / 2, y - size / 4, 4 * scaleFactor, 4 * scaleFactor);
  }
}

function drawSoundMapEnvironmentalZones(voice, section) {
  ...
  for (let z = 0; z < numZones; z++) {
    ...
    // Irregular zone boundary
    beginShape();
    vertex(x, y + h / 4);
    vertex(x + w / 4, y);
    vertex(x + w * 3 / 4, y + h / 6);
    vertex(x + w, y + h / 3);
    vertex(x + w - w / 6, y + h);
    vertex(x + w / 3, y + h - h / 4);
    endShape(CLOSE);
    ...
    textSize(6 * scaleFactor);
    textAlign(CENTER, CENTER);
    const zones = ["forest", "wetland", "meadow", "urban", "shore"];
    text(zones[rndInt(0, zones.length - 1)], x + w / 2, y + h / 2);
  }
}

function drawSoundMapMigrationPaths(voice, section) {
  // Animal/sound migration paths
  const numPaths = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));

  for (let p = 0; p < numPaths; p++) {
    const startX = section.xStart + 15;
    const startY = rnd(voice.yStart + 15, voice.yEnd - 15);

    stroke(features.palette.ink + "70");
    strokeWeight(1 * scaleFactor);
    setLineDash([3 * scaleFactor, 5 * scaleFactor]);
    noFill();
    ...
    for (let s = 0; s < 5; s++) {
      vertex(mx, my);
      mx += rnd(20, 35) * scaleFactor;
      my += rnd(-15, 15) * scaleFactor;
    }
    endShape();

    setLineDash([]);

    // Bird/animal markers along path
    fill(features.palette.ink + "60");
    noStroke();
    for (let b = 0; b < 3; b++) {
      const bx = startX + b * 35 * scaleFactor;
      const by = startY + rnd(-5, 5) * scaleFactor;
      triangle(bx, by, bx - 3 * scaleFactor, by + 2 * scaleFactor, bx - 3 * scaleFactor, by - 2 * scaleFactor);
    }
  }
}

function drawSoundMapEchoLocations(voice, section) {
  // Echo/reverb location markers
  const numEchoes = Math.max(1, Math.floor(rnd(2, 4) * features.densityValue));
  ...
  for (let e = 0; e < numEchoes; e++) {
    ...
    for (let r = 1; r <= 3; r++) {
      const alpha = 255 - r * 60;
      stroke(features.palette.ink + alpha.toString(16).padStart(2, "0"));
      ellipse(x, y, r * size / 2, r * size / 2);
    }
...
function drawSoundMapDensityGradients(voice, section) {
  // Sound density gradient areas
  const numGradients = Math.max(1, Math.floor(rnd(1, 3) * features.densityValue));

  for (let g = 0; g < numGradients; g++) {
    const startX = rnd(section.xStart + 15, section.xEnd - 60);
    const y = rnd(voice.yStart + 15, voice.yEnd - 20);
    const width = rnd(50, 80) * scaleFactor;
    const height = rnd(20, 30) * scaleFactor;

    noStroke();
    // Gradient dots increasing in density
    for (let x = startX; x < startX + width; x += 4 * scaleFactor) {
      const density = (x - startX) / width;
      const numDots = Math.floor(density * 5);
      fill(features.palette.ink + Math.floor(density * 150 + 50).toString(16).padStart(2, "0"));
      for (let d = 0; d < numDots; d++) {
        ellipse(x + rnd(-2, 2) * scaleFactor, y + rnd(0, height), 2 * scaleFactor, 2 * scaleFactor);
      }
    }
...
function drawSoundMapTemporalLayers(voice, section) {
  // Time-based layering
  const numLayers = rndInt(3, 5);
  const x = rnd(section.xStart + 25, section.xEnd - 50);
  const y = rnd(voice.yStart + 15, voice.yEnd - 30);
  const width = rnd(40, 60) * scaleFactor;

  for (let l = 0; l < numLayers; l++) {
    const ly = y + l * 8 * scaleFactor;
    const alpha = 255 - l * 40;

    stroke(features.palette.ink + alpha.toString(16).padStart(2, "0"));
    strokeWeight(1 * scaleFactor);
    line(x, ly, x + width, ly);
    ...
    if (l === 0) {
      ...
      textSize(5 * scaleFactor);
      textAlign(LEFT, BOTTOM);
      text("t₀", x, ly - 2 * scaleFactor);
      textAlign(RIGHT, BOTTOM);
      text("t₁", x + width, ly - 2 * scaleFactor);
    }
  }
}

function drawSoundMapSpatialCoordinates(voice, section) {
  // Coordinate grid overlay
  const x = rnd(section.xStart + 25, section.xEnd - 45);
  const y = rnd(voice.yStart + 15, voice.yEnd - 35);
  const size = rnd(35, 50) * scaleFactor;
  ...
  textSize(6 * scaleFactor);
  textAlign(CENTER, TOP);
  text("x", x + size / 2, y + size + 2 * scaleFactor);
  textAlign(RIGHT, CENTER);
  text("y", x - 4 * scaleFactor, y + size / 2);

  // Point markers
  fill(features.palette.accent);
  for (let p = 0; p < 3; p++) {
    ellipse(x + rnd(0.2, 0.8) * size, y + rnd(0.2, 0.8) * size, 4 * scaleFactor, 4 * scaleFactor);
  }
}

function drawSoundMapListeningPosts(voice, section) {
  // Listening station markers
  const numPosts = Math.max(2, Math.floor(rnd(2, 5) * features.densityValue));
  ...
  for (let p = 0; p < numPosts; p++) {
    ...
    // Headphones symbol
    arc(x, y - size / 6, size, size * 0.8, PI, TWO_PI);
    // Ear cups
    ellipse(x - size / 2, y, size / 4, size / 3);
    ellipse(x + size / 2, y, size / 4, size / 3);
...
function drawModeElements(mode, voice, section) {
  switch (mode) {
    ...
    case "polymorphic":
      // Polymorphic mode v3.24.0 - Logothetis' unique symbol system
      // Primary structural element (choose one)
      const polyPrimary = rndInt(0, 5);
      switch (polyPrimary) {
        case 0: drawPolymorphicLogothetisGlyphs(voice, section); break;
        case 1: drawPolymorphicSoundMasses(voice, section); break;
        case 2: drawPolymorphicTextureFields(voice, section); break;
        case 3: drawPolymorphicTransformChains(voice, section); break;
        case 4: drawPolymorphicExpandingForms(voice, section); break;
        case 5: drawPolymorphicRotationalGlyphs(voice, section); break;
      }
      // Secondary elements (probabilistic layering)
      if (rndBool(0.50)) drawPolymorphicElasticTime(voice, section);
      if (rndBool(0.45)) drawPolymorphicGravityShapes(voice, section);
      if (rndBool(0.40)) drawPolymorphicArticulationSymbols(voice, section);
      if (rndBool(0.38)) drawPolymorphicDynamicCurves(voice, section);
      if (rndBool(0.35)) drawPolymorphicConnectionWebs(voice, section);
      if (rndBool(0.32)) drawPolymorphicIntegrationSigns(voice, section);
      if (rndBool(0.30)) drawPolymorphicDensityGradients(voice, section);
      if (rndBool(0.28)) drawPolymorphicLayeredPlanes(voice, section);
      if (rndBool(0.25)) drawPolymorphicRhythmicCells(voice, section);
      if (rndBool(0.20)) drawPolymorphicCompressingForms(voice, section);
      break;

    case "theater":
      // Theater mode v3.25.0 - Kagel's instrumental theater
      // Primary structural element (choose one)
      const theaterPrimary = rndInt(0, 5);
      switch (theaterPrimary) {
        case 0: drawTheaterStageDirections(voice, section); break;
        case 1: drawTheaterActionSymbols(voice, section); break;
        case 2: drawTheaterObjectNotation(voice, section); break;
        case 3: drawTheaterSpatialPositions(voice, section); break;
        case 4: drawTheaterAbsurdistMarks(voice, section); break;
        case 5: drawTheaterLightingCues(voice, section); break;
      }
      // Secondary elements (probabilistic layering)
      if (rndBool(0.50)) drawTheaterMovementArrows(voice, section);
      if (rndBool(0.45)) drawTheaterGestureLines(voice, section);
      if (rndBool(0.40)) drawTheaterSpeechBubbles(voice, section);
      if (rndBool(0.38)) drawTheaterTimedActions(voice, section);
      if (rndBool(0.35)) drawTheaterPropsSymbols(voice, section);
      if (rndBool(0.32)) drawTheaterEntranceExits(voice, section);
      if (rndBool(0.30)) drawTheaterSynchronizationLines(voice, section);
      if (rndBool(0.28)) drawTheaterPerformerPaths(voice, section);
      if (rndBool(0.25)) drawTheaterAudienceMarks(voice, section);
      if (rndBool(0.20)) drawTheaterDurationBars(voice, section);
      break;

    case "cues":
      // Cues mode v3.26.0 - Christian Wolff's participatory cueing
      // Primary structural element (choose one)
      const cuesPrimary = rndInt(0, 5);
      switch (cuesPrimary) {
        case 0: drawCuesCueSymbols(voice, section); break;
        case 1: drawCuesConditionalBrackets(voice, section); break;
        case 2: drawCuesListeningIndicators(voice, section); break;
        case 3: drawCuesPlayerNumbers(voice, section); break;
        case 4: drawCuesGroupBrackets(voice, section); break;
        case 5: drawCuesAttentionPoints(voice, section); break;
      }
      // Secondary elements (probabilistic layering)
      if (rndBool(0.50)) drawCuesResponseMarkers(voice, section);
      if (rndBool(0.45)) drawCuesCoordinationLines(voice, section);
      if (rndBool(0.42)) drawCuesWaitSigns(voice, section);
      if (rndBool(0.38)) drawCuesReactionArrows(voice, section);
      if (rndBool(0.35)) drawCuesHoldSymbols(voice, section);
      if (rndBool(0.32)) drawCuesReleaseMarks(voice, section);
      if (rndBool(0.30)) drawCuesFlexibleDurations(voice, section);
      if (rndBool(0.28)) drawCuesMuteSymbols(voice, section);
      if (rndBool(0.25)) drawCuesPlaySymbols(voice, section);
      if (rndBool(0.20)) drawCuesPauseIndicators(voice, section);
      break;

    case "indigenous":
      // Indigenous mode v3.27.0 - Raven Chacon's land-based notation
      // Primary structural element (choose one)
      const indigenousPrimary = rndInt(0, 5);
      switch (indigenousPrimary) {
        case 0: drawIndigenousFlagSymbols(voice, section); break;
        case 1: drawIndigenousHorizonLines(voice, section); break;
        case 2: drawIndigenousGatheringCircles(voice, section); break;
        case 3: drawIndigenousCardinalPoints(voice, section); break;
        case 4: drawIndigenousCelestialMarks(voice, section); break;
        case 5: drawIndigenousSeasonalMarks(voice, section); break;
      }
      // Secondary elements (probabilistic layering)
      if (rndBool(0.50)) drawIndigenousLandMarkers(voice, section);
      if (rndBool(0.45)) drawIndigenousWindDirections(voice, section);
      if (rndBool(0.42)) drawIndigenousWaterSymbols(voice, section);
      if (rndBool(0.38)) drawIndigenousFireSymbols(voice, section);
      if (rndBool(0.35)) drawIndigenousEarthSymbols(voice, section);
      if (rndBool(0.32)) drawIndigenousPathwayLines(voice, section);
      if (rndBool(0.30)) drawIndigenousSilenceSpaces(voice, section);
      if (rndBool(0.28)) drawIndigenousVoiceContours(voice, section);
      if (rndBool(0.25)) drawIndigenousDrumPatterns(voice, section);
      if (rndBool(0.20)) drawIndigenousBreathMarks(voice, section);
      break;

    case "deepListening":
      // DeepListening mode v3.28.0 - Pauline Oliveros' meditation-based scores
      // Primary structural element (choose one)
      const deepPrimary = rndInt(0, 5);
      switch (deepPrimary) {
        case 0: drawDeepListeningMandalaCircles(voice, section); break;
        case 1: drawDeepListeningBreathCycles(voice, section); break;
        case 2: drawDeepListeningConcentricRings(voice, section); break;
        case 3: drawDeepListeningRadiatingLines(voice, section); break;
        case 4: drawDeepListeningSonicEnvironment(voice, section); break;
        case 5: drawDeepListeningGroupConnections(voice, section); break;
      }
      // Secondary elements (probabilistic layering)
      if (rndBool(0.50)) drawDeepListeningAwarenessFoci(voice, section);
      if (rndBool(0.45)) drawDeepListeningMeditationText(voice, section);
      if (rndBool(0.42)) drawDeepListeningExpansionArrows(voice, section);
      if (rndBool(0.38)) drawDeepListeningContractionArrows(voice, section);
      if (rndBool(0.35)) drawDeepListeningCenterPoints(voice, section);
      if (rndBool(0.32)) drawDeepListeningPulseMarks(voice, section);
      if (rndBool(0.30)) drawDeepListeningAttentionZones(voice, section);
      if (rndBool(0.28)) drawDeepListeningSilenceFields(voice, section);
      if (rndBool(0.25)) drawDeepListeningListeningDepths(voice, section);
      if (rndBool(0.20)) drawDeepListeningBodyAwareness(voice, section);
      break;

    case "soundMap":
      // SoundMap mode v3.29.0 - Annea Lockwood's geographic notation
      // Primary structural element (choose one)
      const soundMapPrimary = rndInt(0, 5);
      switch (soundMapPrimary) {
        case 0: drawSoundMapRiverLines(voice, section); break;
        case 1: drawSoundMapTopographicContours(voice, section); break;
        case 2: drawSoundMapConfluencePoints(voice, section); break;
        case 3: drawSoundMapEnvironmentalZones(voice, section); break;
        case 4: drawSoundMapSpatialCoordinates(voice, section); break;
        case 5: drawSoundMapWatershedBoundaries(voice, section); break;
      }
      // Secondary elements (probabilistic layering)
      if (rndBool(0.50)) drawSoundMapLocationMarkers(voice, section);
      if (rndBool(0.45)) drawSoundMapSoundSources(voice, section);
      if (rndBool(0.42)) drawSoundMapFlowArrows(voice, section);
      if (rndBool(0.38)) drawSoundMapElevationMarks(voice, section);
      if (rndBool(0.35)) drawSoundMapFieldRecordingSymbols(voice, section);
      if (rndBool(0.32)) drawSoundMapMigrationPaths(voice, section);
      if (rndBool(0.30)) drawSoundMapEchoLocations(voice, section);
      if (rndBool(0.28)) drawSoundMapDensityGradients(voice, section);
      if (rndBool(0.25)) drawSoundMapTemporalLayers(voice, section);
      if (rndBool(0.20)) drawSoundMapListeningPosts(voice, section);
      break;
  }
}
