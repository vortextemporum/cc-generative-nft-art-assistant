// Molecular Brush v2.3.1 - Fast p5.brush Render
...
function renderFast(molecules) {
  ...
  for (let mol of molecules) {
    ...
    // Color must be passed to brush.set(), not brush.stroke()
    ...
    if (mol.path.length > 10 && rnd() < 0.5) {
      ...
      brush.fill(col, floor(mol.alpha * 90));
      brush.circle(pos.x - width/2, pos.y - height/2, weight * rnd(2, 5));
    }
    ...
    brush.fill(col, floor(mol.alpha * 110));
    brush.circle(endPos.x - width/2, endPos.y - height/2, weight * rnd(2, 4));
  }
