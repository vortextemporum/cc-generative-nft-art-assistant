// Molecular Brush v2.6.0 - Framed Edition
// Frame styles, wood textures, and palette-derived colors
...
function renderArtwork() {
  ...
  // Clear canvas with neutral color first
  background(200, 200, 200);

  // Draw frame FIRST (establishes the border)
  drawFrame();

  // Fill content area with paper color (inside the frame)
  push();
  noStroke();
  fill(bg.r, bg.g, bg.b);
  rect(content.left, content.top, content.w, content.h);
  pop();

  // Light paper texture - only inside frame
  brush.noStroke();
  brush.fill(rgbToHex([bg.r - 6, bg.g - 4, bg.b - 2]), 20);
  brush.fillTexture(0.4, 0.2);
  brush.rect(content.left, content.top, content.w, content.h);
