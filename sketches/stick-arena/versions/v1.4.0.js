// Stick Arena v1.4.0 - Ready Fight Edition
// Hash-based stick figure combat inspired by Xiao Xiao and Stick Fight: The Game
...
const STICK_COLORS = [
  { name: "Cyan", rgb: [50, 180, 200], glow: [100, 230, 255] },      // Was black - now highly visible
  { name: "White", rgb: [245, 245, 250], glow: [255, 255, 255] },
  { name: "Crimson", rgb: [200, 50, 50], glow: [255, 100, 100] },
  { name: "Royal", rgb: [60, 90, 200], glow: [120, 160, 255] },
  { name: "Jade", rgb: [50, 170, 90], glow: [100, 255, 150] },
  { name: "Gold", rgb: [220, 180, 60], glow: [255, 230, 120] },
  { name: "Violet", rgb: [160, 70, 180], glow: [220, 140, 255] },
  { name: "Ember", rgb: [240, 140, 50], glow: [255, 190, 110] },
];
...
let gameState = 'waiting';  // 'waiting', 'countdown', 'fighting', 'victory'
let winner = null;
let victoryTimer = 0;
let globalTime = 0;
let countdownTimer = 0;
let countdownPhase = 0;  // 0 = Ready, 1 = Go!
...
class StickFigure {
  ...
  render() {
    ...
    for (let ai of this.afterimages) {
      this.renderBody(ai.x, ai.y, ai.pose, ai.facing, ai.alpha, false);
    }

    // Render outline first for contrast (dark border)
    this.renderBody(this.x, this.y, this.pose, this.facing, 255, true);

    // Render main body on top
    this.renderBody(this.x, this.y, this.pose, this.facing, 255, false);
  }

  renderBody(x, y, pose, facing, alpha, isOutline = false) {
    push();
    translate(x, y);
    scale(facing, 1);

    // === SQUASH & STRETCH ===
    // Calculate based on velocity and action state
    let stretchY = 1.0;
    let stretchX = 1.0;

    if (alpha === 255 && !isOutline) {  // Only for main body, not afterimages or outline
      // Vertical velocity affects stretch
      if (this.vy < -5) {
        // Going up fast = stretch tall
        stretchY = 1.0 + Math.min(Math.abs(this.vy) * 0.015, 0.15);
        stretchX = 1.0 / Math.sqrt(stretchY);  // Preserve volume
      } else if (this.vy > 3 && !this.grounded) {
        // Falling = slight stretch
        stretchY = 1.0 + Math.min(this.vy * 0.01, 0.08);
        stretchX = 1.0 / Math.sqrt(stretchY);
      }

      // Landing impact = squash
      if (this.grounded && Math.abs(this.vx) < 1 && this.stunned > 0) {
        stretchY = 0.9;
        stretchX = 1.1;
      }

      // Attacking = dynamic stretch toward attack direction
      if (this.attacking && this.currentMove) {
        stretchY = 1.05;
        stretchX = 0.97;
      }

      // Blocking/ducking = squash
      if (this.blocking) {
        stretchY = 0.92;
        stretchX = 1.08;
      }
    ...
    // Outline pass: thicker dark stroke for contrast
    if (isOutline) {
      stroke(15, 15, 20, alpha * 0.7);
      ...
      if (features.fx.type === "auras" && alpha === 255) {
        ...
        drawingContext.shadowColor = `rgba(${this.glowColor[0]},${this.glowColor[1]},${this.glowColor[2]},0.6)`;
      }
      ...
      // Stun effect - brief red flash
      if (this.stunned > 0 && this.stunned > 15) {
        stroke(255, 120, 120, alpha);
      }
...
class Environment {
  ...
  generateParticles() {
    ...
    // Reduced particle counts for cleaner look
    const count = type === "rain" ? 40 : type === "data" ? 20 : 15;

    for (let i = 0; i < count; i++) {
      this.particles.push({
        x: rnd(width),
        y: rnd(height),
        vx: rnd(-0.15, 0.15),  // Much slower horizontal drift
        vy: type === "rain" ? rnd(2, 4) : rnd(-0.3, 0.3),  // 70% slower
        size: rnd(2, 4),
        alpha: rnd(30, 80),  // More subtle
  ...
  update() {
    this.time += 0.3;  // Slow down all time-based animations by 70%
    ...
    for (let p of this.particles) {
      ...
      // Data particles change characters (less frequent)
      if (p.char && rnd() < 0.005) {
  ...
  drawLayer(type, offsetX, depth) {
    ...
    switch(type) {
      ...
      case "lanterns":
        for (let i = 0; i < 5; i++) {  // Fewer lanterns
          const lx = i * 170 + 85;
          const ly = height * 0.22 + Math.sin(this.time * 0.008 + i) * 2;  // Gentler sway
          // String
          stroke(50, 40, 30, alpha * 0.6);
          strokeWeight(1);
          line(lx, 0, lx, ly - 20);
          // Lantern glow
          noStroke();
          fill(255, 200, 150, alpha * 0.2);
          ellipse(lx, ly, 35, 45);
          // Lantern body
          fill(180, 50, 30, alpha * 0.8);
          ellipse(lx, ly, 22, 32);
          fill(255, 220, 180, alpha * 0.6);
          ellipse(lx, ly, 12, 17);
        }
        break;
      ...
      case "buildings":
        noStroke();
        for (let i = 0; i < 5; i++) {  // Fewer buildings
          const bx = i * 170 + 20;
          const bh = 180 + i * 40;  // Deterministic heights
          fill(20, 25, 40, alpha * 0.8);
          rect(bx, height - bh - 70, 130, bh);
          // Windows - mostly static, subtle glow
          fill(accent[0], accent[1], accent[2], alpha * 0.25);
          for (let wy = height - bh - 50; wy < height - 90; wy += 28) {
            for (let wx = bx + 15; wx < bx + 120; wx += 22) {
              rect(wx, wy, 10, 14);
            }
          }
        }
      ...
      case "neon_signs":
        strokeWeight(2);
        for (let i = 0; i < 3; i++) {  // Fewer signs
          const sx = i * 260 + 120;
          const sy = height * 0.28;
          // Gentle occasional flicker, not constant
          const flicker = Math.sin(this.time * 0.02 + i * 3) > 0.9 ? 0.5 : 0.8;
          stroke(accent[0], accent[1], accent[2], alpha * 0.6 * flicker);
          // Simple sign shape
          line(sx, sy, sx + 25, sy);
          line(sx + 12, sy, sx + 12, sy + 28);
          line(sx, sy + 28, sx + 25, sy + 28);
        }
        break;

      case "steam_vents":
        noStroke();
        for (let i = 0; i < 3; i++) {  // Fewer vents
          const vx = i * 280 + 100;
          for (let j = 0; j < 5; j++) {  // Less steam puffs
            const py = height - 85 - j * 18 - Math.sin(this.time * 0.015 + i) * 4;  // Slower, smaller movement
            const px = vx + Math.sin(this.time * 0.01 + j) * 5;
            fill(180, 180, 195, alpha * 0.12 * (1 - j / 5));  // More subtle
            ellipse(px, py, 18 + j * 4, 14 + j * 3);
          }
        }
        break;
      ...
      case "data_pillars":
        for (let i = 0; i < 6; i++) {  // Fewer pillars
          const px = i * 140 + 70;
          const ph = 140 + Math.sin(this.time * 0.006 + i) * 15;  // Much slower pulse
          // Data stream
          stroke(accent[0], accent[1], accent[2], alpha * 0.25);
          strokeWeight(2);
          for (let y = height - 70; y > height - 70 - ph; y -= 12) {
            if (Math.sin(y * 0.08 + this.time * 0.02 + i) > 0) {  // Slower pattern
              line(px - 4, y, px + 4, y);
            }
          }
        }
      ...
      case "code_rain":
        textSize(10);
        textFont('monospace');
        fill(accent[0], accent[1], accent[2], alpha * 0.3);  // More subtle
        noStroke();
        for (let i = 0; i < 10; i++) {  // Fewer columns
          const cx = i * 80 + 40;
          for (let j = 0; j < 8; j++) {  // Fewer characters per column
            const cy = ((this.time * 0.5 + j * 40 + i * 23) % (height + 50)) - 25;
            const char = String.fromCharCode(0x30A0 + ((Math.floor(this.time * 0.3) + i + j) % 96));
            text(char, cx, cy);
          }
        }
        break;
      ...
      case "trees":
        // Deterministic tree sizes based on index (not random per frame)
        const treeHeights = [95, 118, 83, 127, 91, 135, 88, 112];
        for (let i = 0; i < 8; i++) {
          const tx = i * 120 + 40;
          const th = treeHeights[i];  // Fixed height per tree
          // Trunk
          stroke(60, 45, 30, alpha);
          strokeWeight(6);
          line(tx, height - 60, tx, height - 60 - th * 0.4);
          // Canopy - gentle sway using slow time
          const sway = Math.sin(this.time * 0.01 + i * 0.7) * 3;
          noStroke();
          fill(40, 80, 45, alpha);
          ellipse(tx + sway, height - 60 - th * 0.6, th * 0.6, th * 0.8);
          fill(50, 100, 55, alpha * 0.7);
          ellipse(tx - 10 + sway * 0.5, height - 60 - th * 0.5, th * 0.4, th * 0.5);
        }
        break;

      case "clouds":
        noStroke();
        fill(255, 255, 255, alpha * 0.25);  // Softer
        for (let i = 0; i < 4; i++) {  // Fewer clouds
          const cx = (i * 220 + this.time * 0.03) % (width + 200) - 100;  // Much slower drift
          const cy = height * 0.12 + i * 25;
          ellipse(cx, cy, 90, 35);
          ellipse(cx + 25, cy - 8, 50, 28);
          ellipse(cx - 20, cy + 4, 45, 25);
        }
        break;
      ...
      case "circuits":
        ...
        strokeWeight(1);
        // Deterministic circuit paths
        const circuitOffsets = [0.15, 0.45, 0.25, 0.55, 0.35, 0.65, 0.20, 0.50, 0.40, 0.30];
        const circuitPaths = [[20,0,20,20,0], [0,20,20,0,-20], [20,20,0,20,20], [0,-20,20,20,0], [20,0,0,20,20]];
        for (let i = 0; i < 10; i++) {
          let cx = i * 100;
          let cy = height * circuitOffsets[i] + height * 0.3;
          beginShape();
          noFill();
          vertex(cx, cy);
          const path = circuitPaths[i % 5];
          for (let j = 0; j < 5; j++) {
            cx += (j % 2 === 0) ? path[j] : 0;
            cy += (j % 2 === 1) ? path[j] : 0;
            vertex(cx, cy);
        ...
        noStroke();
        const nodePositions = [[50,0.35],[150,0.45],[280,0.38],[380,0.52],[470,0.40],[550,0.48],[120,0.55],[220,0.42],[340,0.58],[430,0.35],[520,0.50],[600,0.43],[80,0.62],[260,0.33],[400,0.55]];
        for (let i = 0; i < 15; i++) {
          const [nx, nyRatio] = nodePositions[i];
          ellipse(nx, height * nyRatio, 4, 4);
        }
        break;

      case "capacitors":
        noStroke();
        // Deterministic capacitor positions
        const capOffsets = [15, 65, 30, 80, 45, 55];
        for (let i = 0; i < 6; i++) {
          const cx = i * 150 + 50;
          const cy = height * 0.4 + capOffsets[i];
          fill(40, 40, 50, alpha);
          rect(cx, cy, 20, 40, 3);
      ...
      case "cursor":
        // Slow, subtle cursor movement
        const cursorX = width * 0.55 + Math.sin(this.time * 0.005) * 60;
        const cursorY = height * 0.35 + Math.cos(this.time * 0.004) * 30;
        fill(255, 255, 255, alpha * 0.8);
        noStroke();
        beginShape();
        vertex(cursorX, cursorY);
        vertex(cursorX, cursorY + 16);
        vertex(cursorX + 4, cursorY + 12);
        vertex(cursorX + 8, cursorY + 19);
        vertex(cursorX + 10, cursorY + 18);
        vertex(cursorX + 7, cursorY + 11);
        vertex(cursorX + 11, cursorY + 11);
        endShape(CLOSE);
        break;
    }
...
class InteractiveObject {
  ...
  update(groundY) {
    ...
    // Physics for barrels
    if (this.type === 'barrel' && !this.grounded) {
      this.vy += 0.5;
      this.y += this.vy;
      this.x += this.vx;
      this.vx *= 0.98;
  ...
  render() {
    ...
    switch (this.type) {
      case 'crate':
        ...
        stroke(80, 55, 30);
        strokeWeight(3);
        rect(this.x - this.width/2, this.y - this.height, this.width, this.height, 3);
        // Wood grain lines
        stroke(90, 60, 35);
        strokeWeight(1);
        line(this.x - this.width/2 + 8, this.y - this.height + 5, this.x - this.width/2 + 8, this.y - 5);
        line(this.x + this.width/2 - 8, this.y - this.height + 5, this.x + this.width/2 - 8, this.y - 5);
        // Cross beams
        stroke(80, 55, 30);
        strokeWeight(4);
      ...
      case 'barrel':
        ...
        stroke(50, 50, 55);
        strokeWeight(2);
        ellipse(this.x, this.y - this.height + 8, this.width, 16);
        fill(60, 60, 70);
        rect(this.x - this.width/2, this.y - this.height + 8, this.width, this.height - 16);
        fill(70, 70, 80);
        ellipse(this.x, this.y - 8, this.width, 16);
        // Bands
        stroke(90, 90, 100);
        strokeWeight(3);
        line(this.x - this.width/2, this.y - this.height + 15, this.x + this.width/2, this.y - this.height + 15);
        line(this.x - this.width/2, this.y - 15, this.x + this.width/2, this.y - 15);
        break;
      ...
      case 'platform':
        ...
        stroke(60, 60, 65);
        strokeWeight(2);
        rect(this.x - this.width/2, this.y - this.height, this.width, this.height, 2);
        // Support legs
        fill(60, 60, 65);
        rect(this.x - this.width/2 + 10, this.y, 8, 40);
        rect(this.x + this.width/2 - 18, this.y, 8, 40);
        break;
    }
...
function initializeGame() {
  ...
  objects = [];
  const objectTypes = ['crate', 'barrel', 'platform'];
  const objectWeights = [0.4, 0.4, 0.2];  // 40% crate, 40% barrel, 20% platform
  for (let i = 0; i < features.objects.count; i++) {
    // Weighted random selection
    const roll = R();
    let type;
    if (roll < objectWeights[0]) type = 'crate';
    else if (roll < objectWeights[0] + objectWeights[1]) type = 'barrel';
    else type = 'platform';

    // Position objects across the arena, avoiding fighters
    const ox = 100 + (i * (width - 200) / Math.max(features.objects.count, 1));
    objects.push(new InteractiveObject(type, ox, groundY));
  }
...
function draw() {
  ...
  if (camera.shake > 0.5 && gameState === 'fighting') {
    translate(rnd(-camera.shake, camera.shake), rnd(-camera.shake, camera.shake));
  }
  translate(-width / 2, -height / 2);

  // Update & render environment (always animate background)
  environment.update();
  ...
  // Render objects (behind fighters)
  for (let obj of objects) {
    obj.render();
  }
  ...
  if (gameState === 'waiting') {
    // Frozen first frame - don't update fighters, just render
    for (let f of fighters) {
      f.render();
    }
  } else if (gameState === 'countdown') {
    // Countdown - still frozen, update timer
    countdownTimer++;

    // 60 frames = 1 second at 60fps
    if (countdownTimer < 60) {
      countdownPhase = 0;  // Ready
    } else if (countdownTimer < 120) {
      countdownPhase = 1;  // Go!
    } else {
      gameState = 'fighting';  // Start the fight!
    }
  ...
  // UI overlays (after camera pop, in screen space)
  if (gameState === 'waiting') {
    drawStartButton();
  } else if (gameState === 'countdown') {
    drawCountdown();
  } else if (gameState === 'victory' && victoryTimer > 60) {
    drawVictoryOverlay();
  }
}
...
function drawStartButton() {
  ...
  noStroke();
  fill(230, 70, 70);
  rect(btnX + 4, btnY + 4, btnW - 8, btnH / 2 - 4, 8, 8, 0, 0);
  ...
  textSize(28);
  textStyle(BOLD);
  text("START THE FIGHT", width / 2, height / 2);
  textStyle(NORMAL);

  // Subtle pulse effect
  const pulse = Math.sin(globalTime * 0.08) * 0.1 + 0.9;
  noFill();
...
function drawCountdown() {
  ...
  if (countdownPhase === 0) {
    // "Ready..."
    const alpha = Math.min(countdownTimer * 8, 255);
    const scale = 1 + Math.max(0, 1 - countdownTimer / 15) * 0.5;
    ...
    // "GO!"
    const elapsed = countdownTimer - 60;
    const alpha = Math.min(elapsed * 10, 255);
    const scale = 1 + Math.max(0, 1 - elapsed / 10) * 0.8;
...
function keyPressed() {
  ...
  if ((key === ' ' || keyCode === ENTER) && gameState === 'waiting') {
    startCountdown();
  }
}
